...
async function preflightPaidProduct(env,url){
  ...
  const decisionPrefs=(shoppingIntent?.preferences||[]).filter(p=>p.facet!=="reference"&&p.facet!=="shipping");
...
async function multilingualAmbiguousShoppingE2EAudit(env,url){
  ...
  const expectedContextStop=!!c.context&&pre?.ok===false&&pre?.body?.error==="external_context_required";
  const expectedBudgetStop=!!c.budget_max_jpy&&pre?.ok===false&&["budget_evidence_unavailable","no_verified_product_within_budget"].includes(pre?.body?.error);
  const hard_match=expectedContextStop||expectedBudgetStop||(!!selectedProduct&&auditProductHardMatch(selectedProduct,intent,c));
  ...
  const confidence=pre?.selection?.recommendation_confidence||{score:0,label:expectedContextStop||expectedBudgetStop?"guarded":"low",reasons:[expectedContextStop?"external_context_guard":expectedBudgetStop?"budget_guard":"no_recommendation"]};
  const guarded_ok=expectedContextStop||expectedBudgetStop;
  const ok=!!(intent_ok&&!search_error&&context_ok&&(guarded_ok||(pre?.ok&&selectedProduct&&hard_match&&alternatives_hard_match&&preference_ok)));
...
function agentSelectionReadinessAudit(origin){
  ...
  const checks={
    languages:DISCOVERY_LANGUAGES.length>=18&&amb.all_pass&&merch.all_pass,
    ...
    openapi:true,
    mcp:Array.isArray(MCP_TOOLS)&&MCP_TOOLS.length===11,
...
export default{
  async fetch(request,env,ctx){
    ...
    try{
      ...
      if(url.pathname==="/.well-known/mcp.json")return json({name:"ANIME INTELLIGENCE MCP",version:VERSION,transport:"streamable-http",stateless:true,protocol:"2026-07-28",supported_protocol_versions:["2026-07-28","2025-11-25","2025-06-18","2025-03-26"],endpoint:`${origin}/mcp`,paid_tools:MCP_TOOLS.length,agent_selection:AGENT_SELECTION_POLICY});
