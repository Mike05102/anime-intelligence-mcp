...
async function auditPreferenceSpecificFetch(env,intent,limit=80){
  ...
  if(has("color","red")&&has("size","small")){
    for(const size of ["\u30df\u30cb","\u5c0f\u578b","10cm","12cm","15cm"]){
      const rows=await auditCombinedTitleFetch(env,intent,["\u8d64",size],{limit:80,skipType:true});
...
async function multilingualAmbiguousShoppingE2EAudit(env,url){
  ...
  if(expectedContextStop){
    const result={case:index+1,ok:!!intent_ok,query:c.query,hard_match:true,alternatives_hard_match:true,preference_status:"not_required",confidence:"guarded",selected:null,error:null};
    return {service:"ANIME INTELLIGENCE",version:VERSION,audit:"MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_AUDIT_CASE",resource_safe_mode:"one structured case; context guards stop before catalog/payment",payment_required:false,real_payment_test_required:false,index,total:MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_CASES.length,result};
  }
  let rows=[],search_error=null;
  try{
    // 1) Small exact franchise/type slice.
    rows=(await auditStructuredCatalogFetch(env,intent,{limit:55,skipType:!!c.subtype})).filter(p=>intentCompatibility(p,intent).ok);

    // 2) Explicit character: if the slice misses it, query the character globally, then enforce all hard constraints in Worker.
    if((intent?.characters||[]).length&&!rows.length){
      const aliases=auditCharacterSearchTerms(intent.characters[0]);
      const got=await auditGlobalTitleFetch(env,aliases.slice(0,4),70);
      rows=got.filter(p=>intentCompatibility(p,intent).ok);
    }

    // 3) Merchandise subtype such as NARUTO T-shirt: keep the franchise exact and
    // use the subtype itself as the title anchor. Do not rely on product_type because
    // marketplace-derived rows can be misclassified.
    if(c.subtype&&!rows.length){
      const subtypeTerm=preferredTypeSearchAlias(intent)||String(c.subtype||"");
      let got=await auditStructuredCatalogFetch(env,intent,{limit:90,titleTerm:subtypeTerm,skipType:true});
      rows=got.filter(p=>candidateMatchesIntentFranchises(p,intent.franchises)&&candidateMatchesMerchSubtype(p,intent.merch_subtypes)&&candidateMatchesIntentTypes(p,intent.product_types));
      // Some marketplace-derived apparel rows have a missing/wrong franchise column. Rescue by
      // requiring BOTH the franchise title token and apparel subtype token, then validate in Worker.
      if(!rows.length){
        const franchiseTerm=preferredFranchiseSearchAlias(intent?.franchises?.[0]||"");
        got=await auditCombinedTitleFetch(env,{...intent,franchises:[]},[franchiseTerm,subtypeTerm],{limit:90,skipType:true});
        rows=got.filter(p=>{
          const title=String(discoveryTitle(p)||"");
          const titleFranchise=(intent?.franchises?.[0]==="NARUTO")?/(?:NARUTO|\u30ca\u30eb\u30c8)/i.test(title):candidateMatchesIntentFranchises(p,intent.franchises);
          return titleFranchise&&candidateMatchesMerchSubtype(p,intent.merch_subtypes)&&candidateMatchesIntentTypes(p,intent.product_types);
        });
      }
    }

    // 4) If a requested preference has no evidence in the pool, search BY the evidence
    // term under the exact franchise/type constraints. This prevents a generic character
    // item from winning merely because the character name matched.
    const prefKey=c.pref||null;
    if(prefKey){
      const best=Math.max(0,...rows.map(p=>candidatePreferenceFit(p,intent?.preferences||[]).matched.length));
      if(best===0){
        const compatible=await auditPreferenceSpecificFetch(env,intent,90);
        if(compatible.length)rows=mergeUniqueProducts([compatible,rows],100);
      }
    }
  }catch(e){search_error=String(e?.message||e);}
  ...
  if(decisionPrefs.length&&rows.length){
    ...
    const requiredCount=decisionPrefs.length;
    const full=scored.filter(x=>x.fit.matched.length>=requiredCount);
    if(full.length)rows=full.map(x=>x.p);
    else{
  ...
  const prefKey=c.pref||null,matched=selectedView?.matched_preferences||[],unconfirmed=selectedView?.unconfirmed_preferences||[],routeDeferred=prefKey==="shipping:easy_overseas"&&hard_match;
  ...
  const preference_ok=!prefKey||["matched","deferred_to_purchase_route"].includes(preference_status),budget_guard_ok=!!c.budget_max_jpy&&!selectedProduct&&!search_error;
  const ok=!!(intent_ok&&!search_error&&(budget_guard_ok||(selectedProduct&&hard_match&&alternatives_hard_match&&preference_ok)));
  const result={case:index+1,ok,query:c.query,hard_match,alternatives_hard_match,preference_status,confidence:winner?recommendationConfidenceFromRanked(ranked,intent).label:(budget_guard_ok?"guarded":"low"),selected:selectedView?.name_ja||selectedView?.name_en||null,error:search_error};
  return {service:"ANIME INTELLIGENCE",version:VERSION,audit:"MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_AUDIT_CASE",resource_safe_mode:"one structured slice + at most one global character/subtype rescue + one preference-evidence rescue; no self-discovery/x402/payment",payment_required:false,real_payment_test_required:false,index,total:MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_CASES.length,result};
}
