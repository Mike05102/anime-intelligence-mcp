...
function naturalShoppingIntent(query="",url=null,env=null){
  const raw=String(query||"").trim(),n=foldLatinForSearch(raw),hints=multilingualQueryHints(raw);
  ...
  if(!buyerCountry){
    const countries=[
      ["US",/\b(?:us|u\.s\.|usa|united states|america)\b|\b(?:live|living|ship|deliver|buyer)\s+in\s+(?:the\s+)?us\b/i],
      ["JP",/\b(?:japan|jp|japanese buyer)\b|æ¥æ¬(?:å¨ä½|å½å|ã§è²·|ã¸éé)/i],
      ["GB",/\b(?:uk|u\.k\.|united kingdom|britain)\b/i],
      ["CA",/\bcanada\b/i],["AU",/\baustralia\b/i],["DE",/\bgermany\b/i],["FR",/\bfrance\b/i],["IT",/\bitaly\b/i],["ES",/\bspain\b/i],["KR",/\b(?:korea|south korea)\b/i],["SG",/\bsingapore\b/i]
    ];
    buyerCountry=(countries.find(([,re])=>re.test(raw))||[])[0]||"JP";
  }
  let budgetCurrency=null,budgetAmount=null,budgetJpy=null,budgetEstimated=false;
  const yen=raw.match(/(?:Â¥|ï¿¥|JPY\s*)\s*([0-9][0-9,]*(?:\.\d+)?)|([0-9][0-9,]*(?:\.\d+)?)\s*(?:å|yen\b)/i);
  const usd=raw.match(/(?:\$|USD\s*)\s*([0-9][0-9,]*(?:\.\d+)?)/i);
  if(yen){budgetCurrency="JPY";budgetAmount=Number(String(yen[1]||yen[2]).replace(/,/g,""));budgetJpy=budgetAmount;}
  else if(usd){budgetCurrency="USD";budgetAmount=Number(String(usd[1]).replace(/,/g,""));const fx=envUsdJpyRate(env)||150;budgetJpy=Math.round(budgetAmount*fx);budgetEstimated=!envUsdJpyRate(env);}
  const budgetLimit=/(?:under|below|less than|max(?:imum)?|up to|within|budget|ä»¥å|ä»¥ä¸|ã¾ã§)/i.test(raw);
  const priorities=[];
  const add=x=>{if(!priorities.includes(x))priorities.push(x);};
  if(/(?:cheap|affordable|budget|value|best price|å®ã|ãæé |ã³ã¹ã)/i.test(raw))add("value");
  if(/(?:gift|present|birthday|christmas|ãã¬ã¼ã³ã|ã®ãã|è´ãç©)/i.test(raw))add("gift");
  if(/(?:cute|kawaii|ãããã|å¯æã)/i.test(raw))add("cute");
  if(/(?:rare|scarce|limited|hard to find|å¸å°|éå®|ã¬ã¢)/i.test(raw))add("rarity");
  if(/(?:premium|high end|scale figure|é«ç´|è±ªè¯)/i.test(raw))add("premium");
  if(/(?:popular|best seller|trending|äººæ°|å®çª)/i.test(raw))add("popular");
  if(/(?:where|best place|site|store|shop|seller|buy from|ã©ãã§|è³¼å¥å|ã·ã§ãã|ãµã¤ã)/i.test(raw))add("purchase_route");
  if(/(?:price|cost|worth|cheapest|market value|how much|overpay|ç¸å ´|ä¾¡æ ¼|æå®|ããã|å²é«)/i.test(raw))add("price");
  if(/(?:buy or wait|should i buy (?:this|it|now)|wait|ä»è²·|è²·ãã¹ã|å¾ã¤)/i.test(raw))add("timing");
  const recommendationIntent=/(?:\bi want\b|\blooking for\b|\bfind me\b|\brecommend|what should i buy|which .* should i buy|æ¬²ãã|æ¢ãã¦|ãããã|ä½ãè²·)/i.test(raw)||!!(hints.franchises?.length||hints.product_types?.length);
  return {
    raw_query:raw,
    ...
    budget:budgetAmount?{amount:budgetAmount,currency:budgetCurrency,limit:budgetLimit,jpy_equivalent:budgetJpy,fx_estimated:budgetEstimated}:null,
    priorities
  };
}
...
function naturalPaidRoute(intent,query=""){
  ...
  const multi=["price","timing","purchase_route","rarity"].filter(x=>p.has(x)).length;
  ...
  if(p.has("price"))return "/v1/market";
  return "/v1/identify";
}
...
const NATURAL_SHOPPING_AUDIT_CASES=[
  ...
  {query:"Find the best affordable Pokemon collectible for me",expect:"/v1/identify"},
