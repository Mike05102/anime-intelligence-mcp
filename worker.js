...
function preferenceTargetedQueries(intent,originalQuery=""){
  ...
  // Search each preference separately before combining them. Marketplace titles often carry
  // one evidence term (e.g. æªéå° or èµ¤) but not every adjective from the buyer sentence.
  ...
  // Shipping is a purchase-route property; first make sure a canonical matching product exists.
  if((intent?.preferences||[]).some(p=>p.facet==="shipping"))add([identity,type].filter(Boolean).join(" "));
  add(originalQuery);
  return out.slice(0,5);
}
...
function candidateIntentBonus(p,intent){
  ...
  const prefFit=candidatePreferenceFit(p,intent?.preferences||[]);b.ambiguous_preference_fit=prefFit;const prefEvidenceBoost=prefFit.matched.length*28;const prefUnknownPenalty=prefFit.unknown.length*18;b.preference_evidence_boost=prefEvidenceBoost;b.preference_unknown_penalty=-prefUnknownPenalty;score+=prefFit.score+prefEvidenceBoost-prefUnknownPenalty;
...
async function preflightPaidProduct(env,url){
  ...
  // v3.7.50 preference-aware retrieval: hard identity stays mandatory, but when the current compatible
...
async function multilingualAmbiguousShoppingE2EAudit(env,url){
  ...
  let pre=null,search_error=null;
  try{pre=await preflightPaidProduct(env,u);}catch(e){search_error=String(e?.message||e);}
  const selectedProduct=pre?.product||null;
  const selectedView=pre?.selection?.selected||null;
  const alternatives=Array.isArray(pre?.selection?.alternatives)?pre.selection.alternatives:[];
  const hard_match=!!selectedProduct&&auditProductHardMatch(selectedProduct,intent,c);
  const alternatives_hard_match=alternatives.every(x=>x?.score_breakdown?.intent_compatibility?.ok===true);
  const pref_key=c.pref||null,matched=selectedView?.matched_preferences||[],unconfirmed=selectedView?.unconfirmed_preferences||[];
  const routeDeferred=pref_key==="shipping:easy_overseas"&&hard_match;
  ...
  const preference_ok=!pref_key||preference_status==="matched"||preference_status==="deferred_to_purchase_route"||c.context;
  const context_ok=!c.context||intent?.context_requirements?.includes(c.context);
  const confidence=pre?.selection?.recommendation_confidence||{score:0,label:"low",reasons:["no_recommendation"]};
  const ok=!!(intent_ok&&!search_error&&pre?.ok&&selectedProduct&&hard_match&&alternatives_hard_match&&context_ok&&preference_ok);
  const result={lang:c.lang,query:c.query,ok,expected:c,intent_ok,catalog_result_count:selectedProduct?1:0,search_error,hard_match,alternatives_hard_match,preference_status,context_ok,recommendation_confidence:confidence,selected:selectedView,alternatives:alternatives.slice(0,2)};
  return {service:"ANIME INTELLIGENCE",version:VERSION,audit:"MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_AUDIT_CASE",payment_required:false,real_payment_test_required:false,index,total:MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_CASES.length,result,note:"Resource-safe production-path audit: exactly one live case per Worker request. Each case uses the same preflight recommendation path as paid APIs but stops before x402, so preference-targeted retrieval and hard-constraint guards are genuinely exercised without payment."};
}
