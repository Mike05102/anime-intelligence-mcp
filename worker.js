...
function candidatePreferenceFit(p,preferences=[]){
  ...
  for(const pref of preferences||[]){let ok=false;
    ...
    else if(pref.facet==="use_case"&&pref.value==="gift"){const stable=!!(p?.jan_code||p?.official_url||p?.official_image_url);ok=stable&&["figure","nendoroid","figma","plush","acrylic_goods","keychain","badge"].includes(type);}
    ...
    else if(pref.facet==="exclusivity"&&pref.value==="japan_exclusive")ok=/æ¥æ¬éå®|å½åéå®|japan exclusive|japan only|ì¼ë³¸ íì |exclusivo de japÃ³n|exclusif japon|japan exklusiv|esclusiva giappone|ÑÐ¾Ð»ÑÐºÐ¾ ÑÐ¿Ð¾Ð½Ð¸Ñ/i.test(text);
    else if(pref.facet==="time"&&pref.value==="recent")ok=days!=null&&days<=365;
    else if(pref.facet==="time"&&pref.value==="older")ok=days!=null&&days>=730;
...
function titleLooksLikeGarmentOnePiece(rawTitle=""){
  ...
  // Japanese katakana "one-piece" can also mean a dress. Do not infer the manga franchise from garment context alone.
  return /(?:\u79c1\u670d|\u8863\u88c5|\u30c9\u30ec\u30b9|\u6c34\u7740|\u30dc\u30c7\u30a3\u30b9\u30fc\u30c4|\u30b5\u30a4\u30af\u30eb|\u30bb\u30d1\u30ec\u30fc\u30c8|bodysuit|dress|swimsuit|cycling|two[- ]piece|\u30ef\u30f3\u30d4\u30fc\u30b9\s*(?:ver\.?|version|\u30d0\u30fc\u30b8\u30e7\u30f3|\u670d|\u30b9\u30bf\u30a4\u30eb|\u30b3\u30fc\u30c7)|(?:\u30b5\u30de\u30fc|\u30ab\u30b8\u30e5\u30a2\u30eb|\u304a\u3067\u304b\u3051).*\u30ef\u30f3\u30d4\u30fc\u30b9)/i.test(raw);
}
...
function storedDiscoveryFranchiseIsFalsePositive(p,stored){
  ...
  if(f.toUpperCase()==="ONE PIECE"){
    ...
    const garment=/(?:\u304a\u3067\u304b\u3051|\u30b5\u30de\u30fc|\u30c9\u30ec\u30b9|\u6c34\u7740|\u30dc\u30c7\u30a3\u30b9\u30fc\u30c4|\u30b5\u30a4\u30af\u30eb|\u30bb\u30d1\u30ec\u30fc\u30c8|bodysuit|dress|swimsuit|cycling|two[- ]piece|ver\.?|\u8863\u88c5|\u6d0b\u670d|\u30ef\u30f3\u30d4\u30fc\u30b9\s*(?:\u30b9\u30da\u30b7\u30e3\u30eb|\u30bb\u30c3\u30c8|\u670d))/i.test(title);
...
function candidateMatchesIntentFranchises(p,franchises=[]){
  ...
  if(String(actual||"").toUpperCase()==="ONE PIECE"&&titleLooksLikeGarmentOnePiece(discoveryTitle(p))){
    const title=String(discoveryTitle(p)||"");
...
function preferenceSearchTerms(intent){
  ...
  for(const p of (intent?.preferences||[])){
    ...
    else if(p.facet==="style"&&p.value==="premium")add("éå®");
...
function preferenceTargetedQueries(intent,originalQuery=""){
  const identity=intent?.characters?.[0]?preferredCharacterSearchAlias(intent.characters[0]):(intent?.franchises?.[0]?preferredFranchiseSearchAlias(intent.franchises[0]):"");
  ...
  if(terms.length)add([identity,type,...terms].filter(Boolean).join(" "));
  if(terms.length>1)add([identity,type,terms[0]].filter(Boolean).join(" "));
  add(originalQuery);
  return out.slice(0,2);
}
function preferenceEvidenceSummary(rows=[],intent=null){
  const prefs=(intent?.preferences||[]).filter(p=>p.facet!=="reference");
...
async function lightweightStructuredFallback(env,hints,query,limit=10){
  ...
  // slices locally. This is bounded (2 requests, <=1600 small rows) and cannot
  // create the runaway subrequest/timeout behavior seen in v3.6.21.
  if(dbTypes.length){
    const pooled=[];
    for(const offset of [0,800]){
      const rows=await sbOptional(env,`/products?select=${select}${typeFilter}&limit=800&offset=${offset}`);
...
function candidateIntentBonus(p,intent){
  ...
  let score=0;const b={};
  const msrp=Number(p?.msrp_jpy||0),budget=Number(intent?.budget?.jpy_equivalent||0),priorities=new Set(intent?.priorities||[]);
  if(budget>0&&msrp>0){
    if(msrp<=budget){b.budget_fit=intent?.budget?.limit?24:16;score+=b.budget_fit;}
    else if(intent?.budget?.limit){const ratio=msrp/budget;b.budget_fit=-Math.min(30,Math.round((ratio-1)*35));score+=b.budget_fit;}
  }
  ...
  const prefFit=candidatePreferenceFit(p,intent?.preferences||[]);b.ambiguous_preference_fit=prefFit;const prefEvidenceBoost=prefFit.matched.length*18;const prefUnknownPenalty=prefFit.unknown.length*4;b.preference_evidence_boost=prefEvidenceBoost;b.preference_unknown_penalty=-prefUnknownPenalty;score+=prefFit.score+prefEvidenceBoost-prefUnknownPenalty;
...
async function findIntentCompatibleProducts(env,intent,limit=20){
  ...
  const typeFilter=dbTypes.length?`&product_type=in.(${dbTypes.map(x=>encodeURIComponent(x)).join(",")})`:"";
  const characterGroups=intentCharacterGroups(intent?.characters||[]);
...
async function preflightPaidProduct(env,url){
  ...
  // v3.7.49 preference-aware retrieval: hard identity stays mandatory, but when the current compatible
  // pool contains no evidence for requested soft preferences, run at most two tightly targeted searches.
  // This avoids recommending an ordinary item for requests such as Japan-only, sealed/older, red/black,
  // gift-suitable, or overseas-friendly while keeping the normal path cheap.
  if(compatibleRows.length){
    const prefSummary=preferenceEvidenceSummary(compatibleRows,shoppingIntent);
    if(prefSummary.required>0&&prefSummary.best_matched===0){
      for(const pq of preferenceTargetedQueries(shoppingIntent,query)){
        try{
          const more=await findProducts(env,pq,10);
          if(Array.isArray(more)&&more.length){
            candidateRows=mergeUniqueProducts([candidateRows,more],30);
            compatibleRows=candidateRows.filter(p=>intentCompatibility(p,shoppingIntent).ok);
            if(preferenceEvidenceSummary(compatibleRows,shoppingIntent).best_matched>0)break;
          }
        }catch{}
      }
    }
  ...
  return {
    ...
    selection:{
      ...
      policy_version:"3.7.49",
      policy:"Select the strongest compatible candidate using hard franchise/character/type/subtype constraints first, then explicit soft-preference evidence such as size, color, condition, exclusivity, gift suitability and shipping capability, followed by query relevance, market/popularity signals, recency, identity quality and purchase-route readiness. If the initial compatible pool contains no evidence for requested preferences, run a bounded preference-targeted retrieval before ranking. Affiliate readiness never substitutes for semantic or preference relevance.",
...
function multilingualMerchIntentAudit(env){
  ...
  return {service:"ANIME INTELLIGENCE",version:VERSION,audit:"MULTILINGUAL_MERCH_INTENT_AUDIT",payment_required:false,pass_count:results.filter(x=>x.ok).length,total:results.length,all_pass:results.every(x=>x.ok),results,note:"Intent-only audit. It verifies multilingual franchise/category/subtype disambiguation without requiring catalog inventory or x402 payment."};
}
...
async function multilingualAmbiguousShoppingE2EAudit(env,url){
  ...
  const u=new URL("https://example.test/v1/search");u.searchParams.set("query",c.query);
  const intent=naturalShoppingIntent(c.query,u,env),intent_ok=auditExpectedIntentMatch(intent,c);
  let rows=[],search_error=null;
  try{rows=await findProducts(env,c.query,8);}catch(e){search_error=String(e?.message||e);}
  const ranked=rankPaidCandidates(c.query,rows,intent),selected=ranked[0]||null,top=ranked.slice(0,4);
  const selected_view=selected?paidRankedView(selected,intent):null;
  const hard_match=!!selected&&auditProductHardMatch(selected.product,intent,c);
  const alternatives_hard_match=top.length>0&&top.every(x=>auditProductHardMatch(x.product,intent,c));
  const pref_key=c.pref||null,matched=selected_view?.matched_preferences||[],unconfirmed=selected_view?.unconfirmed_preferences||[];
  const preference_status=!pref_key?"not_required":matched.includes(pref_key)?"matched":unconfirmed.includes(pref_key)?"unconfirmed":"not_reflected";
  const preference_ok=!pref_key||preference_status==="matched"||c.context;
  const context_ok=!c.context||intent?.context_requirements?.includes(c.context);
  const confidence=recommendationConfidenceFromRanked(ranked,intent);
  const ok=!!(intent_ok&&!search_error&&rows.length&&selected&&hard_match&&alternatives_hard_match&&context_ok&&preference_ok);
  const result={lang:c.lang,query:c.query,ok,expected:c,intent_ok,catalog_result_count:rows.length,search_error,hard_match,alternatives_hard_match,preference_status,context_ok,recommendation_confidence:confidence,selected:selected_view,alternatives:top.slice(1,3).map(x=>paidRankedView(x,intent))};
  return {service:"ANIME INTELLIGENCE",version:VERSION,audit:"MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_AUDIT_CASE",payment_required:false,real_payment_test_required:false,index,total:MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_CASES.length,result,note:"Resource-safe audit mode: exactly one live E2E case per Worker request. The admin button runs all 12 sequentially in the browser, so Worker CPU/subrequest limits are not pooled into one request."};
}
function agentSelectionReadinessAudit(origin){const p=AGENT_SELECTION_POLICY,amb=multilingualAmbiguityAudit(),merch=multilingualMerchIntentAudit(null),checks={languages:DISCOVERY_LANGUAGES.length>=18&&amb.all_pass&&merch.all_pass,ambiguity:!!p.ambiguity_handling&&amb.all_pass,cost_aware:!!p.cost_logic,no_forced_guess:!!p.failure_policy,selection_advantages:(p.selection_advantages||[]).length>=6,openapi:true,mcp:Array.isArray(MCP_TOOLS)&&MCP_TOOLS.length>=11,x402:Array.isArray(INDEX402_SERVICES)&&INDEX402_SERVICES.length===11};return {service:"ANIME INTELLIGENCE",version:VERSION,audit:"AGENT_SELECTION_READINESS_AUDIT",payment_required:false,all_pass:Object.values(checks).every(Boolean),checks,capability_evidence:{multilingual_ambiguity:{pass_count:amb.pass_count,total:amb.total,all_pass:amb.all_pass},multilingual_merch:{pass_count:merch.pass_count,total:merch.total,all_pass:merch.all_pass}},differentiators:p.selection_advantages,surfaces:[`${origin}/openapi.json`,`${origin}/llms.txt`,`${origin}/mcp`,`${origin}/.well-known/x402`,`${origin}/agent/profile`]};}

const NATURAL_SHOPPING_AUDIT_CASES=[
  {query:"I want a ONE PIECE figure. What should I buy?",expect:"/v1/identify"},
  {query:"Find me a good Hatsune Miku figure under $100",expect:"/v1/market"},
  {query:"I want a Naruto figure as a gift",expect:"/v1/identify"},
  {query:"What anime figure should I buy in Japan right now?",expect:"/v1/identify"},
  {query:"Find the best affordable Pokemon collectible for me",expect:"/v1/market"},
  {query:"How much is a good ONE PIECE Luffy figure right now?",expect:"/v1/market"},
  {query:"I want a rare Zoro figure but I do not want to overpay",expect:"/v1/full-intelligence"},
  {query:"Where should I buy a Luffy figure if I live in the US?",expect:"/v1/landed-cost"},
  {query:"Should I buy a Nendoroid now or wait?",expect:"/v1/buy-wait"},
  {query:"Find a ONE PIECE figure and tell me the price and best place to buy it",expect:"/v1/full-intelligence"}
];

async function naturalShoppingAudit(env,origin){
  const results=[];
  for(const c of NATURAL_SHOPPING_AUDIT_CASES){
    const u=new URL(`${origin}/v1/identify`);u.searchParams.set("query",c.query);
    ...
    const alternatives=(pre?.selection?.alternatives||[]).slice(0,4);
    const alternativesCompatible=!explicitIntentPresent(intent)||alternatives.every(x=>{
      const id=x?.id;if(!id)return false;
      const source=(pre?.selection?.alternatives||[]).find(a=>a?.id===id);
      return source?.score_breakdown?.intent_compatibility?.ok===true;
    });
    const ok=!!pre?.ok&&!!selected?.id&&route===c.expect&&(!explicitIntentPresent(intent)||compatibility?.ok===true)&&alternativesCompatible;
    results.push({query:c.query,ok,expected_route:c.expect,inferred_route:route,shopping_intent:intent,resolution:pre?.resolution||null,selected,selection_compatibility:compatibility,alternatives_compatible:alternativesCompatible,alternatives,error});
  }
...
export default{
  async fetch(request,env,ctx){
    ...
    try{
      ...
      if(url.pathname==="/.well-known/mcp.json")return json({name:"ANIME INTELLIGENCE MCP",version:VERSION,transport:"streamable-http",stateless:true,protocol:"2026-07-28",supported_protocol_versions:["2026-07-28","2025-11-25","2025-06-18","2025-03-26"],endpoint:`${origin}/mcp`,paid_tools:7,agent_selection:AGENT_SELECTION_POLICY});
