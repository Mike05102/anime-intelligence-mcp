...
async function multilingualAmbiguousShoppingE2EAudit(env){
  const results=[];
  for(const c of MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_CASES){
    ...
    try{rows=await findProducts(env,c.query,12);}catch(e){search_error=String(e?.message||e);}
    const ranked=rankPaidCandidates(c.query,rows,intent),selected=ranked[0]||null,top=ranked.slice(0,5);
    ...
    results.push({lang:c.lang,query:c.query,ok,expected:c,intent_ok,catalog_result_count:rows.length,search_error,hard_match,alternatives_hard_match,preference_status,context_ok,recommendation_confidence:confidence,selected:selected_view,alternatives:top.slice(1,4).map(x=>paidRankedView(x,intent))});
  }
  const hard_pass_count=results.filter(x=>x.intent_ok&&x.hard_match&&x.alternatives_hard_match).length;
  const preference_matched_count=results.filter(x=>x.preference_status==="matched").length;
  const preference_required_count=results.filter(x=>x.expected?.pref).length;
  return {service:"ANIME INTELLIGENCE",version:VERSION,audit:"MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_AUDIT",payment_required:false,real_payment_test_required:false,pass_count:results.filter(x=>x.ok).length,total:results.length,all_pass:results.every(x=>x.ok),hard_constraint_pass_count:hard_pass_count,preference_matched_count,preference_required_count,results,note:"End-to-end free audit: multilingual vague intent -> live catalog retrieval -> ranking -> hard-constraint safety -> soft-preference evidence -> recommendation confidence. No x402 settlement is entered. A soft preference marked unconfirmed does not count as a full pass; this intentionally exposes catalog/ranking evidence gaps."};
}
  ...
  async fetch(request,env,ctx){
    ...
    try{
      ...
      if(url.pathname.startsWith("/admin/")){
        ...
        if(url.pathname==="/admin/multilingual-ambiguous-shopping-e2e-audit"&&request.method==="GET")return json(await multilingualAmbiguousShoppingE2EAudit(env));
