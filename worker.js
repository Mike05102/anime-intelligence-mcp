...
async function fastAuditCatalogCandidates(env,intent,limit=50){
  ...
  const dbTypes=[...new Set((intent?.product_types||[]).flatMap(t=>PRODUCT_TYPE_SEARCH_EQUIVALENTS[t]||[t]))];
  const typeFilter=dbTypes.length?`&product_type=in.(${dbTypes.map(x=>encodeURIComponent(x)).join(",")})`:"";
  const terms=[];const add=v=>{v=safeSearchTerm(String(v||"").trim());if(v&&!terms.includes(v))terms.push(v);};
  for(const c of (intent?.characters||[]).slice(0,1))add(preferredCharacterSearchAlias(c)||c);
  if(!terms.length)for(const f of (intent?.franchises||[]).slice(0,1))add(preferredFranchiseSearchAlias(f)||f);
  if(!terms.length)for(const t of (intent?.product_types||[]).slice(0,1))add(t);
  for(const term of terms.slice(0,1)){
    const or=["canonical_name_ja","canonical_name_en","franchise","series"].map(k=>`${k}.ilike.*${term}*`).join(",");
    const rows=await sbOptional(env,`/products?select=*${typeFilter}&or=(${encodeURIComponent(or)})&limit=80`);
    if(Array.isArray(rows)&&rows.length)groups.push(rows);
  }
  let merged=mergeUniqueProducts(groups,100).filter(p=>intentCompatibility(p,intent).ok);
  const decisionPrefs=(intent?.preferences||[]).filter(p=>p.facet!=="reference"&&p.facet!=="shipping");
  if(decisionPrefs.length&&merged.length){
    const best=Math.max(0,...merged.map(p=>candidatePreferenceFit(p,decisionPrefs).matched.length));
    if(best===0){
      const pt=preferenceSearchTerms(intent)[0];
      if(pt){
        const term=safeSearchTerm(pt),or=["canonical_name_ja","canonical_name_en","series"].map(k=>`${k}.ilike.*${term}*`).join(",");
        const rows=await sbOptional(env,`/products?select=*${typeFilter}&or=(${encodeURIComponent(or)})&limit=80`);
        if(Array.isArray(rows)&&rows.length)merged=mergeUniqueProducts([merged,rows],120).filter(p=>intentCompatibility(p,intent).ok);
      }
    }
  }
...
async function multilingualAmbiguousShoppingE2EAudit(env,url){
  ...
  const expectedContextStop=!!c.context&&intent?.context_requirements?.includes(c.context);
  if(expectedContextStop){
    const result={lang:c.lang,query:c.query,ok:!!intent_ok,expected:c,intent_ok,catalog_result_count:0,search_error:null,hard_match:true,alternatives_hard_match:true,preference_status:"not_required",context_ok:true,recommendation_confidence:{score:1,label:"guarded",reasons:["external_context_guard"]},selected:null,alternatives:[]};
    return {service:"ANIME INTELLIGENCE",version:VERSION,audit:"MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_AUDIT_CASE",payment_required:false,real_payment_test_required:false,index,total:MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_CASES.length,result,note:"FAST SAFE live-catalog audit. Uses Supabase catalog retrieval + production intent/compatibility/ranking logic only; no self-discovery, x402, Phantom, payment verification or settlement."};
  }
  let rows=[],search_error=null;
  try{rows=await fastAuditCatalogCandidates(env,intent,50);}catch(e){search_error=String(e?.message||e);}
  if(c.budget_max_jpy&&rows.length){
    const known=rows.map(p=>({p,b:budgetConstraintStatus(p,intent)})).filter(x=>x.b.known);
    const within=known.filter(x=>x.b.ok).map(x=>x.p);
    if(within.length)rows=within;else if(known.length)rows=[];
  }
  const ranked=rankPaidCandidates(c.query,rows,intent),winner=ranked[0]||null;
  const selectedProduct=winner?.product||null,selectedView=winner?paidRankedView(winner,intent):null;
  const alternatives=ranked.slice(1,4).map(x=>paidRankedView(x,intent));
  ...
  const preference_ok=!pref_key||["matched","deferred_to_purchase_route"].includes(preference_status);
  const budget_guard_ok=!!c.budget_max_jpy&&!selectedProduct&&!search_error;
  const ok=!!(intent_ok&&!search_error&&(budget_guard_ok||(selectedProduct&&hard_match&&alternatives_hard_match&&preference_ok)));
  const result={lang:c.lang,query:c.query,ok,expected:c,intent_ok,catalog_result_count:rows.length,search_error,hard_match,alternatives_hard_match,preference_status,context_ok:true,recommendation_confidence:winner?recommendationConfidenceFromRanked(ranked,intent):{score:0,label:budget_guard_ok?"guarded":"low",reasons:[budget_guard_ok?"budget_guard":"no_catalog_match"]},selected:selectedView,alternatives:alternatives.slice(0,2)};
  return {service:"ANIME INTELLIGENCE",version:VERSION,audit:"MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_AUDIT_CASE",payment_required:false,real_payment_test_required:false,index,total:MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_CASES.length,result,note:"FAST SAFE live-catalog audit. Uses Supabase catalog retrieval + production intent/compatibility/ranking logic only; no self-discovery, x402, Phantom, payment verification or settlement."};
}
