...
async function auditStructuredCatalogFetch(env,intent,opts={}){
  ...
  params.set("select","*");
  params.set("limit",String(Math.max(1,Math.min(Number(opts.limit||120),160))));
...
async function fastAuditCatalogCandidates(env,intent,limit=50){
  ...
  const groups=[];
  const base=await auditStructuredCatalogFetch(env,intent,{limit:120});
  ...
  if((intent?.characters||[]).length&&!merged.length){
    ...
    const rows=await auditStructuredCatalogFetch(env,intent,{limit:80,titleTerm:alias});
  ...
  if(decisionPrefs.length){
    ...
    if(best===0){
      ...
      for(const q of targeted.slice(0,2)){
        ...
        const rows=await auditStructuredCatalogFetch(env,intent,{limit:80,titleTerm:evidence});
