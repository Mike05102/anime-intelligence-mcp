...
async function countryAwareRoutingAudit(env,query="åé³ãã¯"){
  const q=String(query||"").trim()||"åé³ãã¯";
  ...
  for(const country of countries){
    ...
    results.push({
      ...
      checks:{best_route_practical_when_direct_option_exists:bestIsPractical,no_invented_cross_border_landed_total:noInventedCrossBorderTotal},
  ...
  const allPass=results.every(r=>r.selection_policy==="purchase_feasibility_then_ease_then_known_cost"&&r.checks.best_route_practical_when_direct_option_exists&&r.checks.no_invented_cross_border_landed_total);
  return {
    service:"ANIME INTELLIGENCE",version:VERSION,audit:"COUNTRY_AWARE_PURCHASE_ROUTING_AUDIT",payment_required:false,real_payment_test_required:false,query:q,
  ...
  async fetch(request,env,ctx){
    ...
    try{
      ...
      if(url.pathname.startsWith("/admin/")){
        ...
        if(url.pathname==="/admin/country-routing-audit"&&request.method==="GET")return json(await countryAwareRoutingAudit(env,url.searchParams.get("query")||"åé³ãã¯"));
