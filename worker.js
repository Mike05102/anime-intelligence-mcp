...
// v3.7.92 â isolate known legacy self-tests without permanently blacklisting a generic Node/NL fingerprint.
// The fingerprint below was produced by our own pre-v3.7.92 test runner. It is treated as test traffic only
// for events at or before the final known self-test window. Future traffic with the same coarse fingerprint
// remains eligible to count as external unless it carries an explicit test marker.
const LEGACY_SELF_TEST_FINGERPRINT_CUTOFFS={
  "03ed7b5e9a95f7f08a07af23":"2026-09-24T08:28:51.132Z"
};
function isLegacySelfTestFingerprintEvent(x){
  const fp=String(x?.metadata?.source_fingerprint||"");
  const cutoff=LEGACY_SELF_TEST_FINGERPRINT_CUTOFFS[fp];
  if(!cutoff)return false;
  const ts=Date.parse(String(x?.occurred_at||"")),end=Date.parse(cutoff);
  return Number.isFinite(ts)&&Number.isFinite(end)&&ts<=end;
}
function isAdminE2ETestEvent(x,env){
  const meta=x?.metadata||{},ref=String(meta.referer||"").toLowerCase(),source=String(meta.source_class||"");
  if(source==="admin_or_test"||String(meta.test_mode||"").startsWith("admin_")||String(meta.test_mode||"").startsWith("test_")||ref.includes("/admin")||isLegacySelfTestFingerprintEvent(x))return true;
...
function isContextlessSyntheticShoppingCall(x){
  const m=x?.metadata||{};
  if(m.id_present||m.canonical_product_id)return false;
  const q=String(m.intent_query||m.query_text||"").trim();
  if(!q)return false;
  const deictic=/\b(this|that|this exact|this cheap|this figure|this nendoroid|this listing|this prize|this item|the exact figure|the exact nendoroid)\b/i.test(q);
  const genericOnly=/^(what is the real total cost for a buyer in [a-z ]+\??|when does preorder close for this figure\??|buy or wait for this .*figure\??)$/i.test(q);
  const concreteId=/https?:\/\/|\bjan\s*[:#-]?\s*\d{8,14}\b|\b\d{12,14}\b|\b[A-Z0-9_-]{8,}\b/i.test(q);
  return (deictic||genericOnly)&&!concreteId;
}

async function readAffiliateTelemetry(env){
  const limit=5000;
  const filter=encodeURIComponent('(affiliate_link_served,affiliate_click)');
  try{
    const rows=await sb(env,`/api_events?select=*&event_type=in.${filter}&order=occurred_at.desc&limit=${limit}`);
    return {ok:true,rows:Array.isArray(rows)?rows:[],error:null};
  }catch(e){return {ok:false,rows:[],error:safeError(e)};}
}
...
async function revenueMetrics(env){
  ...
  // v3.7.93: only events carrying, or request-linked to, a real query/canonical-id are allowed
  // into the confirmed external shopping funnel. Bare non-test probes are reported separately.
  const confirmedIntentRequestIds=new Set(realShoppingIntentCalls.map(x=>String(x?.metadata?.request_id||"")).filter(Boolean));
  const hasConfirmedShoppingIntent=x=>{
    if(!isExternalCommercialEvent(x))return false;
    const m=x?.metadata||{},rid=String(m.request_id||"");
    return !!(m.query_present||m.id_present||m.canonical_product_id||(rid&&confirmedIntentRequestIds.has(rid)));
  };
  const externalSelected=selected.filter(hasConfirmedShoppingIntent),externalGateEntered=gateEntered.filter(hasConfirmedShoppingIntent),externalConfigErrors=configErrors.filter(hasConfirmedShoppingIntent),externalPaymentRequired=paymentRequired.filter(hasConfirmedShoppingIntent),externalAttempts=attempts.filter(hasConfirmedShoppingIntent),externalVerified=verified.filter(hasConfirmedShoppingIntent);
  const nonTestProbeGateEntered=gateEntered.filter(x=>isExternalCommercialEvent(x)&&!hasConfirmedShoppingIntent(x));
  const nonTestProbePaymentRequired=paymentRequired.filter(x=>isExternalCommercialEvent(x)&&!hasConfirmedShoppingIntent(x));
  ...
  const testPaid=paid.filter(x=>isAdminE2ETestEvent(x,env)),externalPaid=paid.filter(hasConfirmedShoppingIntent);
  ...
  const recentHistoryTestPaid=recentHistoryPaid.filter(x=>isAdminE2ETestEvent(x,env)),recentHistoryExternalPaid=recentHistoryPaid.filter(hasConfirmedShoppingIntent);
  ...
  // v3.7.92 â funnel_stage is now the genuine external-shopping funnel only. Discovery crawlers,
  // admin/self-tests and legacy test fingerprints cannot advance or block this stage.
  const stage=!telemetry.ok?"telemetry_unavailable":(realShoppingIntentCalls.length===0)?(calls.length?"no_confirmed_external_shopping_intent":"no_external_api_traffic"):(externalSelected.length===0)?"external_product_intent_but_no_canonical_selection":(externalGateEntered.length===0)?"external_canonical_selected_but_x402_gate_not_entered":(externalPaymentRequired.length===0)?(externalConfigErrors.length?"external_x402_configuration_error_before_402":"external_x402_gate_entered_but_402_not_issued"):(externalAttempts.length===0)?"external_402_issued_but_no_payment_retry":(externalVerified.length===0)?"external_payment_retry_received_but_not_verified":(externalPaid.length===0)?"external_payment_verified_but_not_settled":"external_revenue_confirmed";
...
/* =========================================================
   RAKUTEN ICHIBA LIVE SEARCH + AFFILIATE ROUTING v3.7.87
   Uses the official 2026-07-01 Rakuten Ichiba Item Search API when
   RAKUTEN_APPLICATION_ID + RAKUTEN_ACCESS_KEY are configured.
   RAKUTEN_AFFILIATE_ID is optional and, when present, asks Rakuten
   to return affiliate URLs for live results. Pre-generated affiliate
   links remain supported as a fallback.
========================================================= */

const RAKUTEN_ICHIBA_SEARCH_ENDPOINT="https://openapi.rakuten.co.jp/ichibams/api/IchibaItem/Search/20260701";

function rakutenConfigured(env){return !!String(env.RAKUTEN_AFFILIATE_ID||"").trim()||rakutenWebServiceConfigured(env);}
function rakutenWebServiceConfigured(env){return !!String(env.RAKUTEN_APPLICATION_ID||"").trim()&&!!String(env.RAKUTEN_ACCESS_KEY||"").trim();}

function rakutenSearchQuery(product){
  return [product?.canonical_name_ja,product?.canonical_name_en,product?.model_number,product?.jan_code].map(x=>String(x||"").trim()).find(Boolean)||"anime figure";
}
function rakutenSearchQueries(product){
  const vals=[product?.jan_code,product?.canonical_name_ja,product?.canonical_name_en,[product?.canonical_name_en,product?.model_number].filter(Boolean).join(" ")];
  const seen=new Set(),out=[];
  for(const v of vals){const q=String(v||"").replace(/\s+/g," ").trim();if(!q)continue;const key=q.toLowerCase();if(seen.has(key))continue;seen.add(key);out.push(q);}
  return out.slice(0,4);
}
...
function rakutenApiItems(body){
  const raw=Array.isArray(body?.Items)?body.Items:Array.isArray(body?.items)?body.items:[];
  return raw.map(x=>x?.Item||x?.item||x).filter(Boolean);
}

function normalizeRakutenLiveOffer(item,product,analysis,affiliateIdPresent){
  const price=Number(item?.itemPrice??item?.itemPriceMin1??item?.itemPriceMin2??item?.itemPriceMin3);
  if(!Number.isFinite(price)||price<=0)return null;
  const affiliateUrl=String(item?.affiliateUrl||"").trim();
  const itemUrl=String(item?.itemUrl||"").trim();
  const url=(affiliateUrl&&isOfficialRakutenAffiliateUrl(affiliateUrl))?affiliateUrl:itemUrl;
  if(!url)return null;
  return {source:"rakuten",seller:item?.shopName||item?.shopCode||"Rakuten Ichiba",title:item?.itemName||product?.canonical_name_ja||product?.canonical_name_en||null,item_code:item?.itemCode||null,price_jpy:Math.round(price),shipping_jpy:null,total_price_jpy:Math.round(price),affiliate_url:url,image_url:item?.mediumImageUrls?.[0]?.imageUrl||item?.smallImageUrls?.[0]?.imageUrl||null,match_score:analysis?.score??null,match_basis:analysis?.basis||"live_rakuten_identity",link_source:affiliateUrl?"rakuten_web_service_affiliate_url":"rakuten_web_service_item_url",affiliate_ready:!!affiliateUrl&&affiliateIdPresent,live_api:true};
}

async function rakutenApiSearch(env,product){
  if(!rakutenWebServiceConfigured(env))return {configured:false,returned:0,offers:[],attempts:[],error:null};
  const applicationId=String(env.RAKUTEN_APPLICATION_ID||"").trim(),accessKey=String(env.RAKUTEN_ACCESS_KEY||"").trim(),affiliateId=String(env.RAKUTEN_AFFILIATE_ID||"").trim();
  const attempts=[],seen=new Map();let returned=0,lastError=null;
  for(const query of rakutenSearchQueries(product)){
    try{
      const u=new URL(RAKUTEN_ICHIBA_SEARCH_ENDPOINT);
      ...
      const r=await fetch(u.toString(),{headers:{accept:"application/json","accessKey":accessKey}});
      const text=await r.text();let body=null;try{body=text?JSON.parse(text):null}catch{}
      if(!r.ok){const msg=body?.error_description||body?.error||text.slice(0,240)||`HTTP ${r.status}`;attempts.push({query,status:r.status,ok:false,error:msg});lastError=msg;if(r.status===429||r.status>=500)continue;break;}
      const items=rakutenApiItems(body);returned+=items.length;let accepted=0;
      for(const item of items){const a=yahooIdentityAnalysis(product,{name:item?.itemName||"",janCode:null});if(!a.accepted)continue;const offer=normalizeRakutenLiveOffer(item,product,a,!!affiliateId);if(!offer)continue;const key=String(offer.item_code||offer.affiliate_url||offer.title||"");const prev=seen.get(key);if(!prev||Number(offer.total_price_jpy)<Number(prev.total_price_jpy))seen.set(key,offer);accepted++;}
      attempts.push({query,status:r.status,ok:true,returned:items.length,accepted});
      if(seen.size>=5)break;
    }catch(e){lastError=safeError(e);attempts.push({query,status:0,ok:false,error:lastError});}
  }
  const offers=[...seen.values()].sort((a,b)=>(a.total_price_jpy||Infinity)-(b.total_price_jpy||Infinity)).slice(0,12);
  return {configured:true,returned,offers,attempts,error:offers.length?null:lastError};
}

async function rakutenSearch(env,product){
  const registered=registeredRakutenAffiliateOffers(product),affiliateIdPresent=!!String(env.RAKUTEN_AFFILIATE_ID||"").trim(),apiConfigured=rakutenWebServiceConfigured(env);
  let live={configured:false,returned:0,offers:[],attempts:[],error:null};
  if(apiConfigured)live=await rakutenApiSearch(env,product);
  const byKey=new Map();
...
function normalizeBuyerCountry(v="JP"){
  const c=String(v||"JP").trim().toUpperCase();
  return /^[A-Z]{2}$/.test(c)?c:"JP";
}

const GOODSMILE_EUROPE_COUNTRIES=new Set(["AL","AD","AT","BY","BE","BA","BG","HR","CY","CZ","DK","EE","FI","FR","DE","GR","HU","IS","IE","IT","LV","LI","LT","LU","MT","MD","MC","ME","NL","MK","NO","PL","PT","RO","SM","RS","SK","SI","ES","SE","CH","UA","GB","UK","VA"]);
const GOODSMILE_OCEANIA_COUNTRIES=new Set(["AU","NZ","FJ","PG","SB","VU","WS","TO"]);
const GOODSMILE_SE_ASIA_COUNTRIES=new Set(["BN","KH","ID","LA","MY","MM","PH","SG","TH","TL","VN"]);
const GOODSMILE_LATAM_COUNTRIES=new Set(["AR","BO","BR","CL","CO","CR","CU","DO","EC","SV","GT","HT","HN","NI","PA","PY","PE","UY","VE"]);
const GOODSMILE_MIDDLE_EAST_COUNTRIES=new Set(["BH","IQ","IL","JO","KW","LB","OM","QA","SA","AE","YE"]);
const GOODSMILE_AFRICA_COUNTRIES=new Set(["DZ","AO","BJ","BW","BF","BI","CM","CV","CF","TD","KM","CG","CD","CI","DJ","EG","GQ","ER","SZ","ET","GA","GM","GH","GN","GW","KE","LS","LR","LY","MG","MW","ML","MR","MU","MA","MZ","NA","NE","NG","RW","ST","SN","SC","SL","SO","ZA","SS","SD","TZ","TG","TN","UG","ZM","ZW"]);

function goodSmileShippingUsdForCountry(country="US"){
  const c=normalizeBuyerCountry(country);
  if(c==="JP"||c==="TW")return null;
  if(c==="US")return 46;
  if(c==="CA")return 12;
  if(c==="KR"||c==="HK")return 15;
  if(GOODSMILE_EUROPE_COUNTRIES.has(c)||GOODSMILE_OCEANIA_COUNTRIES.has(c))return 15;
  if(GOODSMILE_SE_ASIA_COUNTRIES.has(c)||c==="MX"||c==="IN"||GOODSMILE_MIDDLE_EAST_COUNTRIES.has(c))return 8;
  if(GOODSMILE_LATAM_COUNTRIES.has(c))return 10;
  if(GOODSMILE_AFRICA_COUNTRIES.has(c))return 12;
  return 18;
}

function isGoodSmileProduct(product){
  const blob=[product?.manufacturer,product?.brand,product?.canonical_name_ja,product?.canonical_name_en,product?.official_url,product?.metadata?.official_english_url].filter(Boolean).join(" ").toLowerCase();
  return /good\s*smile|ã°ããã¹ãã¤ã«/.test(blob)||/goodsmile\.com/.test(blob);
}

function goodSmileOfficialUrl(product){
  const candidates=[product?.metadata?.official_english_url,product?.official_url].filter(Boolean).map(String);
  return candidates.find(x=>/https?:\/\/(?:www\.)?goodsmile\.com\//i.test(x))||null;
}

function officialPurchaseOffersForProduct(product,fxInfo={}){
  const offers=[];
  if(isGoodSmileProduct(product)){
    const exactUrl=goodSmileOfficialUrl(product);
    const msrp=Number(product?.msrp_jpy);
    offers.push({
      source:"goodsmile_official",
      seller:"Good Smile Company Online Store",
      title:cleanOfficialTitle(product?.canonical_name_en||product?.canonical_name_ja||"Good Smile product"),
      asking_price_jpy:null,
      shipping_jpy:null,
      total_price_jpy:null,
      price_jpy:null,
      reference_price_jpy:Number.isFinite(msrp)&&msrp>0?msrp:null,
      price_basis:Number.isFinite(msrp)&&msrp>0?"jp_msrp_reference_not_destination_checkout_price":"destination_checkout_price_not_verified",
      currency:null,
      original_currency:null,
      url:exactUrl||"https://www.goodsmile.com/en/",
      condition:"new",
      availability:"destination_region_login_confirmation_required",
      observed_at:new Date().toISOString(),
      fetched_at:new Date().toISOString(),
      affiliate:false,
      affiliate_source:null,
      match_score:exactUrl?100:85,
      match_basis:exactUrl?"official_exact_product_url":"official_manufacturer_route",
      practical_route_only:true,
      market_metadata:{
        merchant:"goodsmile_official",
        exact_product_url:!!exactUrl,
        international_store:true,
        checkout_currency_non_jp:"USD",
        payment_methods_non_jp:["credit_card","paypal"],
        account_required:true,
        region_product_availability_confirmation_required:true,
        shipping_policy_source:"https://www.goodsmile.com/en/usersguide07",
        payment_policy_source:"https://www.goodsmile.com/en/usersguide03",
        account_policy_source:"https://www.goodsmile.com/en/usersguide01",
        policy_checked_at:"2026-09-22",
        fx_rate_usdjpy:Number.isFinite(Number(fxInfo?.rate))?Number(fxInfo.rate):null,
        fx_source:fxInfo?.source||null
      }
    });
  }
  return offers;
...
function materializeDestinationOffer(offer,buyerCountry="JP"){
  if(!offer)return offer;
  const country=normalizeBuyerCountry(buyerCountry);
  if(String(offer.source||"").toLowerCase()!=="goodsmile_official")return {...offer};
  const fx=Number(offer?.market_metadata?.fx_rate_usdjpy);
  if(country==="JP"){
    const reference=Number(offer.reference_price_jpy);
    const shipping=660;
    return {...offer,shipping_jpy:shipping,shipping_original:660,shipping_currency:"JPY",asking_price_jpy:Number.isFinite(reference)?reference:null,total_price_jpy:Number.isFinite(reference)?reference+shipping:null,price_jpy:Number.isFinite(reference)?reference+shipping:null,price_basis:Number.isFinite(reference)?"jp_official_msrp_plus_standard_shipping_reference":"jp_checkout_price_not_verified"};
  }
  const shippingUsd=goodSmileShippingUsdForCountry(country);
  const shippingJpy=Number.isFinite(shippingUsd)&&Number.isFinite(fx)?Math.round(shippingUsd*fx):null;
  // For non-Japan accounts Good Smile charges the product in USD. Do not pretend the JP MSRP is the checkout item price.
  return {...offer,shipping_jpy:shippingJpy,shipping_original:shippingUsd,shipping_currency:"USD",asking_price_jpy:null,total_price_jpy:null,price_jpy:null,price_basis:"non_jp_checkout_item_price_not_verified",market_metadata:{...(offer.market_metadata||{}),destination_country:country,regional_shipping_usd:shippingUsd}};
}

function sourcePurchaseProfile(source,buyerCountry="JP"){
  const country=normalizeBuyerCountry(buyerCountry),src=String(source||"unknown").toLowerCase();
  const isJP=country==="JP";
  if(src==="yahoo_shopping"){
    return isJP
      ? {source:src,country_fit:"high",country_fit_score:96,direct_shipping:"domestic",language_support:"japanese_first",payment_compatibility:"high_for_japan",account_difficulty:"medium",proxy_required:false,purchase_ease_score:90,trust_score:84,route_type:"direct_domestic",destination_confirmation_required:false}
      : {source:src,country_fit:"low",country_fit_score:35,direct_shipping:"not_verified_for_destination",language_support:"japanese_first",payment_compatibility:"destination_support_not_verified",account_difficulty:"high_for_non_japanese_buyer",proxy_required:true,purchase_ease_score:30,trust_score:82,route_type:"proxy_or_forwarder_likely",destination_confirmation_required:true};
  }
  if(src==="rakuten"){
    return isJP
      ? {source:src,country_fit:"high",country_fit_score:95,direct_shipping:"domestic",language_support:"japanese_first",payment_compatibility:"high_for_japan",account_difficulty:"medium",proxy_required:false,purchase_ease_score:92,trust_score:86,route_type:"direct_domestic",destination_confirmation_required:false}
      : {source:src,country_fit:"medium_low",country_fit_score:45,direct_shipping:"seller_specific_not_verified",language_support:"japanese_first",payment_compatibility:"seller_specific_not_verified",account_difficulty:"high_for_non_japanese_buyer",proxy_required:true,purchase_ease_score:38,trust_score:84,route_type:"direct_if_supported_otherwise_proxy",destination_confirmation_required:true};
  }
  if(src==="ebay"){
    return isJP
      ? {source:src,country_fit:"medium",country_fit_score:72,direct_shipping:"listing_specific_not_verified",language_support:"international_english",payment_compatibility:"international_friendly",account_difficulty:"low_medium",proxy_required:false,purchase_ease_score:76,trust_score:78,route_type:"direct_if_listing_ships_to_destination",destination_confirmation_required:true}
      : {source:src,country_fit:"high",country_fit_score:88,direct_shipping:"listing_specific_not_verified",language_support:"international_english",payment_compatibility:"international_friendly",account_difficulty:"low_medium",proxy_required:false,purchase_ease_score:86,trust_score:78,route_type:"direct_if_listing_ships_to_destination",destination_confirmation_required:true};
  }
  if(src==="zenmarket_proxy"){
    return isJP
      ? {source:src,country_fit:"low",country_fit_score:35,direct_shipping:"unnecessary_for_japan_buyer",language_support:"multilingual",payment_compatibility:"international_friendly",account_difficulty:"low_medium_account_required",proxy_required:true,purchase_ease_score:45,trust_score:88,route_type:"verified_proxy_international",destination_confirmation_required:true}
      : {source:src,country_fit:"high",country_fit_score:90,direct_shipping:"proxy_warehouse_then_international",language_support:"multilingual",payment_compatibility:"credit_card_debit_paypal_crypto_bank_transfer",account_difficulty:"low_medium_account_required",proxy_required:true,purchase_ease_score:84,trust_score:88,route_type:"verified_proxy_international",destination_confirmation_required:true};
  }
  if(src==="goodsmile_official"){
    return isJP
      ? {source:src,country_fit:"high",country_fit_score:98,direct_shipping:"domestic",language_support:"japanese_and_english",payment_compatibility:"major_cards_and_store_checkout",account_difficulty:"low_medium_account_required",proxy_required:false,purchase_ease_score:92,trust_score:98,route_type:"official_direct",destination_confirmation_required:false}
      : {source:src,country_fit:"high",country_fit_score:96,direct_shipping:"verified_by_merchant_region_policy",language_support:"english",payment_compatibility:"credit_card_and_paypal",account_difficulty:"low_medium_account_required",proxy_required:false,purchase_ease_score:91,trust_score:98,route_type:"official_direct_international",destination_confirmation_required:true};
  }
  if(src==="official"){
    return {source:src,country_fit:isJP?"high":"medium",country_fit_score:isJP?90:60,direct_shipping:isJP?"domestic":"site_specific_not_verified",language_support:"site_specific",payment_compatibility:"site_specific",account_difficulty:"site_specific",proxy_required:!isJP,purchase_ease_score:isJP?82:50,trust_score:95,route_type:isJP?"direct_domestic":"official_site_destination_check_required",destination_confirmation_required:!isJP};
  }
...
function zenMarketProxyOfferForDomesticOffer(offer,buyerCountry="US"){
  const country=normalizeBuyerCountry(buyerCountry);
  if(country==="JP"||!offer)return null;
  const src=String(offer.source||"").toLowerCase();
  if(src!=="yahoo_shopping"&&src!=="rakuten")return null;
  const item=Number(offer.asking_price_jpy??offer.total_price_jpy??offer.price_jpy);
  return {
    source:"zenmarket_proxy",
    seller:"ZenMarket proxy shopping",
    title:offer.title||"Japanese marketplace item",
    asking_price_jpy:null,
    shipping_jpy:null,
    total_price_jpy:null,
    price_jpy:null,
    reference_price_jpy:Number.isFinite(item)&&item>0?item:null,
    price_basis:"japan_item_price_reference_plus_unverified_proxy_and_international_shipping_fees",
    currency:null,
    original_currency:"JPY",
    url:"https://zenmarket.jp/en/shopping-japan",
    source_product_url:offer.url||null,
    source_marketplace:src,
    source_seller:offer.seller||null,
    condition:offer.condition||null,
    availability:"proxy_quote_required",
    observed_at:offer.observed_at||null,
    fetched_at:new Date().toISOString(),
    affiliate:false,
    affiliate_source:null,
    match_score:offer.match_score??null,
    match_basis:"verified_proxy_route_for_exact_japanese_listing",
    practical_route_only:true,
    market_metadata:{
      merchant:"zenmarket_proxy",
      source_product_url:offer.url||null,
      source_marketplace:src,
      source_item_price_jpy:Number.isFinite(item)&&item>0?item:null,
      proxy_service:true,
      multilingual_support:true,
      payment_methods:["credit_card","debit_card","paypal","cryptocurrency","bank_transfer"],
      international_shipping:true,
      account_required:true,
      service_fee_floor_jpy:300,
      service_fee_exact_not_verified:true,
      international_shipping_cost_not_verified:true,
      import_tax_duty_not_verified:true,
      policy_source:"https://zenmarket.jp/en/shopping-japan",
      policy_checked_at:"2026-09-22"
    }
  };
}

function uniquePurchaseOffers(offers=[]){
  const out=[],seen=new Set();
  for(const o of offers){
    if(!o)continue;
    const total=Number(o.total_price_jpy);
    if((!Number.isFinite(total)||total<=0)&&!o.practical_route_only)continue;
    const key=String(o.url||`${o.source}|${o.seller||""}|${o.title||""}|${Number.isFinite(total)?total:"unpriced"}`);
    if(seen.has(key))continue;seen.add(key);out.push(o);
  }
  return out;
}

function purchaseRouteTradeoff(route,cheapest,buyerCountry){
  if(!route)return null;
  const country=normalizeBuyerCountry(buyerCountry),extra=cheapest&&Number.isFinite(route.total_price_jpy)&&Number.isFinite(cheapest.total_price_jpy)?Math.max(0,route.total_price_jpy-cheapest.total_price_jpy):null;
  if(String(route.source||"").toLowerCase()==="zenmarket_proxy")return `Selected because it converts the exact Japan-market listing into a multilingual international purchase route for a ${country} buyer. The Japanese item price is known, but proxy service fees, international shipping and import charges remain unverified until quote/checkout.`;
  if(route.proxy_required)return `Lower apparent price may require a proxy/forwarder for a ${country} buyer; destination eligibility and final proxy fees must be confirmed before purchase.`;
  if(extra>0)return `Selected over the apparent cheapest offer because it has a better purchase-fit/ease profile for a ${country} buyer; price premium versus the cheapest known offer is ${Math.round(extra)} JPY before unknown destination taxes/fees.`;
  return `Selected as the strongest current balance of purchase feasibility, ease and known price for a ${country} buyer.`;
}
...
function assessPurchaseRoute(offer,buyerCountry="JP",cheapestKnown=null){
  ...
  if(offer.shipping_jpy==null)blockers.push("destination_shipping_cost_not_verified");
  if(offer.market_metadata?.region_product_availability_confirmation_required)blockers.push("product_region_availability_must_be_confirmed");
  if(String(offer.price_basis||"").includes("not_verified"))blockers.push("destination_checkout_item_price_not_verified");
  if(country!=="JP")blockers.push("destination_tax_duty_or_brokerage_not_verified");
  const directness=profile.route_type==="verified_proxy_international"?"proxy_assisted_international":profile.proxy_required?"proxy_likely":profile.destination_confirmation_required?"direct_if_destination_supported":"direct";
  let recommendedAction=profile.proxy_required?"Use a proxy/forwarder only after comparing its fees against direct international routes.":profile.destination_confirmation_required?`Confirm this exact listing ships to ${country} and accepts the buyer's payment method before checkout.`:"Direct purchase route; verify stock and checkout total before paying.";
  if(String(offer.source||"").toLowerCase()==="goodsmile_official"&&country!=="JP")recommendedAction=`Use the Good Smile international checkout for ${country}; sign in to confirm this product is available in-region. Shipping policy is known, but the destination checkout item price and import charges must be confirmed before paying.`;
  if(String(offer.source||"").toLowerCase()==="zenmarket_proxy"&&country!=="JP")recommendedAction=`Open ZenMarket and submit the source Japanese product URL for a quote. The proxy handles the Japanese purchase, multilingual account/payment flow and international forwarding; verify service fee, international shipping and destination import charges before paying.`;
...
function countryAwarePurchaseRouting(obs=[],rakuten={offers:[]},buyerCountry="JP",officialOffers=[],product=null){
  ...
  const domesticProxySeeds=country==="JP"?[]:rawBase.filter(o=>["yahoo_shopping","rakuten"].includes(String(o.source||"").toLowerCase())).filter(o=>Number.isFinite(Number(o.total_price_jpy))&&Number(o.total_price_jpy)>0).sort((a,b)=>Number(a.total_price_jpy)-Number(b.total_price_jpy)).slice(0,3);
...
const AGENT_SELECTION_POLICY={
  ...
  routing_rules:{
    ...
    best_place:"Use when the user is ready to buy and needs the best buyer-country-aware purchase route, balancing actual purchasability, language/payment friction, proxy need and known price.",
...
function requestTelemetry(request,url=null){
  ...
  const testHeader=(request.headers.get("x-anime-intelligence-test")||request.headers.get("x-ai-test-mode")||"").slice(0,120);
  const low=ua.toLowerCase(),ref=referer.toLowerCase(),clientLow=clientName.toLowerCase();let source_class="unknown";
  if(testHeader)source_class="admin_or_test";
  else if(/agent402/.test(low)||/agent402\.tools/.test(ref)||/agent402/.test(clientLow))source_class="agent402";
  ...
  const test_mode=testHeader?(testHeader.startsWith("admin_")||testHeader.startsWith("test_")?testHeader:`test_${testHeader}`):(u.searchParams.get("ai_e2e")==="1"?"admin_x402_e2e":null);
...
function sanitizeAffiliateRouting(shaped,origin,productId){
  // v3.7.93: AI/API responses do not redistribute the Rakuten affiliate URL itself.
  // They point to our public, registered media page; the unmodified Rakuten URL lives on that page.
  const purchasePageUrl=productId?`${origin}/shop/product/${encodeURIComponent(productId)}`:null;
  if(purchasePageUrl)shaped.purchase_page_url=purchasePageUrl;
  if(shaped.routing?.rakuten?.offers){shaped.routing.rakuten.offers=shaped.routing.rakuten.offers.map(offer=>{const x={...offer,purchase_page_url:purchasePageUrl,affiliate_via_public_page:true};delete x.affiliate_url;delete x.purchase_url;return x;});if(shaped.routing.rakuten.best){shaped.routing.rakuten.best={...shaped.routing.rakuten.best,url:purchasePageUrl,purchase_page_url:purchasePageUrl,affiliate_via_public_page:true};delete shaped.routing.rakuten.best.affiliate_url;}}
  if(shaped.best_place?.source==="rakuten"){shaped.best_place.url=purchasePageUrl;shaped.best_place.purchase_page_url=purchasePageUrl;shaped.best_place.affiliate=true;shaped.best_place.affiliate_via_public_page=true;}
  if(shaped.affiliate?.rakuten_candidates){shaped.affiliate.rakuten_candidates=shaped.affiliate.rakuten_candidates.map(offer=>{const x={...offer,purchase_page_url:purchasePageUrl,affiliate_via_public_page:true};delete x.affiliate_url;delete x.purchase_url;return x;});}
  return shaped;
}


/* =========================================================
   PUBLIC SHOP / RAKUTEN REGISTERED MEDIA v3.7.93
   - Public human-readable media for Rakuten Affiliate site registration.
   - API returns this page URL, not the Rakuten affiliate URL itself.
   - Registered Rakuten URL is rendered directly and unmodified as destination.
========================================================= */
function publicHtmlEscape(v){return String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));}
function publicShopShell(title,body){return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="index,follow"><title>${publicHtmlEscape(title)} | ANIME INTELLIGENCE</title><style>body{margin:0;background:#0a0b0d;color:#f5f7fa;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}main{max-width:900px;margin:auto;padding:24px}.brand{font-size:26px;font-weight:900;letter-spacing:.2px}.sub,.notice{color:#aeb4be;line-height:1.7}.notice{font-size:13px;border-top:1px solid #2b2e34;margin-top:28px;padding-top:18px}.card{background:#15171b;border:1px solid #2d3036;border-radius:16px;padding:18px;margin:14px 0}.grid{display:grid;grid-template-columns:120px 1fr;gap:18px}.thumb{width:120px;height:120px;object-fit:contain;background:white;border-radius:12px}.btn{display:inline-block;background:#e83e8c;color:white;text-decoration:none;font-weight:900;padding:14px 18px;border-radius:12px;margin:8px 8px 8px 0}.btn2{background:#e9eef5;color:#101217}.price{font-size:22px;font-weight:900}.meta{color:#aeb4be;font-size:14px;line-height:1.6}input{width:100%;box-sizing:border-box;padding:15px;border-radius:12px;border:1px solid #3a3e46;background:#111318;color:white;font-size:16px}button{padding:14px 18px;border:0;border-radius:12px;font-weight:900;margin-top:10px}a{color:inherit}@media(max-width:600px){main{padding:18px}.grid{grid-template-columns:88px 1fr}.thumb{width:88px;height:88px}}</style></head><body><main><div class="brand">ANIME INTELLIGENCE</div><p class="sub">\u65e5\u672c\u30a2\u30cb\u30e1\u30fb\u30d5\u30a3\u30ae\u30e5\u30a2\uff0f\u30ad\u30e3\u30e9\u30af\u30bf\u30fc\u30b0\u30c3\u30ba\u306e\u8cfc\u5165\u60c5\u5831\u3092\u3001\u5546\u54c1\u5358\u4f4d\u3067\u6574\u7406\u3059\u308b\u516c\u958b\u30b7\u30e7\u30c3\u30d4\u30f3\u30b0\u30da\u30fc\u30b8\u3067\u3059\u3002</p>${body}<p class="notice">\u5e83\u544a\u306b\u3064\u3044\u3066\uff1a\u5f53\u30b5\u30a4\u30c8\u306f\u697d\u5929\u30a2\u30d5\u30a3\u30ea\u30a8\u30a4\u30c8\u3092\u5229\u7528\u3057\u3066\u3044\u307e\u3059\u3002\u697d\u5929\u5e02\u5834\u3078\u306e\u4e00\u90e8\u30ea\u30f3\u30af\u306f\u30a2\u30d5\u30a3\u30ea\u30a8\u30a4\u30c8\u30ea\u30f3\u30af\u3067\u3059\u3002\u4fa1\u683c\u30fb\u5728\u5eab\u30fb\u9001\u6599\u30fb\u8ca9\u58f2\u6761\u4ef6\u306f\u9077\u79fb\u5148\u3067\u5fc5\u305a\u3054\u78ba\u8a8d\u304f\u3060\u3055\u3044\u3002ANIME INTELLIGENCE\u306eAPI\u8ab2\u91d1\u3068\u306f\u5225\u306e\u4ed5\u7d44\u307f\u3067\u3059\u3002</p></main></body></html>`;}
async function loadProductById(env,id){const clean=String(id||"").trim();if(!clean)return null;const rows=await sbOptional(env,`/products?select=*&id=eq.${encodeURIComponent(clean)}&limit=1`);return Array.isArray(rows)?rows[0]||null:null;}
function publicProductName(p){return cleanNullableTitle(p?.canonical_name_ja)||cleanNullableTitle(p?.canonical_name_en)||"\u5546\u54c1";}
function publicProductCard(p,origin){const name=publicProductName(p),img=p?.official_image_url?`<img class="thumb" src="${publicHtmlEscape(p.official_image_url)}" alt="${publicHtmlEscape(name)}">`:"";return `<div class="card"><div class="grid"><div>${img}</div><div><b>${publicHtmlEscape(name)}</b><div class="meta">${publicHtmlEscape(p?.product_type||"")} ${p?.manufacturer?" / "+publicHtmlEscape(p.manufacturer):""}${p?.jan_code?" / JAN "+publicHtmlEscape(p.jan_code):""}</div><a class="btn btn2" href="${origin}/shop/product/${encodeURIComponent(p.id)}">\u5546\u54c1\u60c5\u5831\u3092\u898b\u308b</a></div></div></div>`;}
async function publicShopLanding(env,origin,url){const q=String(url.searchParams.get("q")||"").trim();let body=`<h1>\u30a2\u30cb\u30e1\u30b0\u30c3\u30ba\u3092\u63a2\u3059</h1><form method="get" action="/shop"><input name="q" value="${publicHtmlEscape(q)}" placeholder="\u4f8b\uff1a\u521d\u97f3\u30df\u30af \u30d5\u30a3\u30ae\u30e5\u30a2 / ONE PIECE \u30b0\u30c3\u30ba"><button type="submit">\u691c\u7d22</button></form>`;if(q){const rows=await findProducts(env,q,20);body+=`<h2>\u691c\u7d22\u7d50\u679c ${rows.length}\u4ef6</h2>`+(rows.length?rows.map(p=>publicProductCard(p,origin)).join(""):`<div class="card">\u8a72\u5f53\u5546\u54c1\u304c\u898b\u3064\u304b\u308a\u307e\u305b\u3093\u3067\u3057\u305f\u3002</div>`);}else{body+=`<div class="card"><b>\u516c\u958b\u30e1\u30c7\u30a3\u30a2</b><p class="meta">AI\u30a8\u30fc\u30b8\u30a7\u30f3\u30c8\u304b\u3089\u6848\u5185\u3055\u308c\u305f\u5546\u54c1\u30da\u30fc\u30b8\u3084\u3001\u691c\u7d22\u7d50\u679c\u304b\u3089\u5404\u5546\u54c1\u306e\u8cfc\u5165\u60c5\u5831\u3092\u78ba\u8a8d\u3067\u304d\u307e\u3059\u3002\u697d\u5929\u30a2\u30d5\u30a3\u30ea\u30a8\u30a4\u30c8\u767b\u9332\u6e08\u307f\u306e\u5546\u54c1\u3067\u306f\u697d\u5929\u5e02\u5834\u3078\u306e\u30ea\u30f3\u30af\u3092\u8868\u793a\u3057\u307e\u3059\u3002</p></div>`;}return publicShopShell("\u30a2\u30cb\u30e1\u30b0\u30c3\u30ba\u691c\u7d22",body);}
async function publicShopProduct(env,origin,productId,ctx=null){const product=await loadProductById(env,productId);if(!product)return null;const name=publicProductName(product),offers=registeredRakutenAffiliateOffers(product).filter(x=>isOfficialRakutenAffiliateUrl(x.affiliate_url)),obs=await getObservations(env,product.id),market=marketView(product,obs),img=product.official_image_url?`<img class="thumb" src="${publicHtmlEscape(product.official_image_url)}" alt="${publicHtmlEscape(name)}">`:"";let body=`<p><a href="/shop">\u2190 \u5546\u54c1\u691c\u7d22\u3078</a></p><div class="card"><div class="grid"><div>${img}</div><div><h1>${publicHtmlEscape(name)}</h1><div class="meta">${publicHtmlEscape(product.product_type||"")}${product.manufacturer?" / "+publicHtmlEscape(product.manufacturer):""}${product.jan_code?" / JAN "+publicHtmlEscape(product.jan_code):""}</div>${market.lowest_total_price_jpy!=null?`<p class="price">\u89b3\u6e2c\u6700\u5b89 \u00a5${Number(market.lowest_total_price_jpy).toLocaleString("ja-JP")}</p><div class="meta">Yahoo/eBay\u7b49\u306e\u76f4\u8fd1\u89b3\u6e2c\u3002\u9001\u6599\u7b49\u304c\u672a\u78ba\u8a8d\u306e\u5834\u5408\u304c\u3042\u308a\u307e\u3059\u3002</div>`:"<p class=\"meta\">\u73fe\u5728\u4fa1\u683c\u306e\u78ba\u5b9a\u60c5\u5831\u306f\u3042\u308a\u307e\u305b\u3093\u3002</p>"}</div></div></div>`;if(offers.length){body+=`<h2>\u697d\u5929\u5e02\u5834</h2>`+offers.slice(0,10).map((o,i)=>`<div class="card"><b>${publicHtmlEscape(o.title||o.seller||"\u697d\u5929\u5e02\u5834\u306e\u5546\u54c1")}</b>${o.price_jpy?`<div class="price">\u00a5${Number(o.price_jpy).toLocaleString("ja-JP")}</div>`:""}<div class="meta">\u767b\u9332\u6e08\u307f\u697d\u5929\u30a2\u30d5\u30a3\u30ea\u30a8\u30a4\u30c8\u30ea\u30f3\u30af${o.registered_at?" / \u767b\u9332\u65e5 "+publicHtmlEscape(String(o.registered_at).slice(0,10)):""}</div><a class="btn" href="${publicHtmlEscape(o.affiliate_url)}" onclick="try{navigator.sendBeacon('/shop/affiliate-click?product_id=${encodeURIComponent(product.id)}&offer=${i}','')}catch(e){fetch('/shop/affiliate-click?product_id=${encodeURIComponent(product.id)}&offer=${i}',{method:'POST',keepalive:true}).catch(()=>{})}">\u697d\u5929\u5e02\u5834\u3067\u898b\u308b</a></div>`).join("")+`<script>(function(){try{navigator.sendBeacon('/shop/affiliate-impression?product_id=${encodeURIComponent(product.id)}&count=${offers.length}','')}catch(e){fetch('/shop/affiliate-impression?product_id=${encodeURIComponent(product.id)}&count=${offers.length}',{method:'POST',keepalive:true}).catch(()=>{})}})();<\/script>`;}else{body+=`<div class="card"><b>\u697d\u5929\u30ea\u30f3\u30af\u306f\u672a\u767b\u9332\u3067\u3059</b><p class="meta">\u697d\u5929\u5e02\u5834\u306e\u767b\u9332\u6e08\u307f\u30a2\u30d5\u30a3\u30ea\u30a8\u30a4\u30c8\u30ea\u30f3\u30af\u304c\u3042\u308b\u5546\u54c1\u3060\u3051\u3001\u3053\u306e\u516c\u958b\u30da\u30fc\u30b8\u306b\u8868\u793a\u3057\u307e\u3059\u3002</p></div>`;}if(market.best_place?.url&&market.best_place?.source!=="rakuten")body+=`<h2>\u305d\u306e\u4ed6\u306e\u5e02\u5834</h2><div class="card"><b>${publicHtmlEscape(market.best_place.source||"market")}</b><div class="price">\u00a5${Number(market.best_place.total_price_jpy||market.best_place.price_jpy||0).toLocaleString("ja-JP")}</div><a class="btn btn2" href="${publicHtmlEscape(market.best_place.url)}">\u8ca9\u58f2\u30da\u30fc\u30b8\u3092\u898b\u308b</a></div>`;return publicShopShell(name,body);}
async function handlePublicAffiliateMetric(env,url,eventType){const pid=String(url.searchParams.get("product_id")||"").trim()||null;const count=Number(url.searchParams.get("count")||0)||null,offer=Number(url.searchParams.get("offer")||0);await logEvent(env,eventType,{endpoint:eventType==="affiliate_click"?"/shop/affiliate-click":"/shop/affiliate-impression",product_id:pid,metadata:{source:"rakuten",mode:"public_registered_media",count,offer_index:Number.isFinite(offer)?offer:null,version:VERSION}});return new Response(null,{status:204,headers:corsHeaders({"cache-control":"no-store"})});}
...
async function preflightPaidProduct(env,url){
  ...
  // Broad natural-language paid queries are allowed to resolve to one recommended canonical
  // product before x402. This is intentionally separate from BUY-WAIT, which still requires
  // exact identity. Hard constraints and evidence guards remain mandatory.
  const broadAutoSelection=!cleanJan(query)&&!/[A-Z]{1,5}[-_ ]?\d{2,}/i.test(query)&&
    !(shoppingIntent?.context_requirements||[]).length&&
    strictCatalogPreferences(shoppingIntent).length===0&&
    ((shoppingIntent?.characters||[]).length>0||(shoppingIntent?.franchises||[]).length>0||(shoppingIntent?.product_types||[]).length>0||query.length<=40);
  ...
  if(narutoTshirtFastPath){
    ...
    rows=await findProducts(env,retrievalQuery,broadAutoSelection?20:10);
    if((!Array.isArray(rows)||!rows.length)){
      const fallback=await commercialFallbackProducts(env,retrievalQuery,broadAutoSelection?20:10);
  ...
  const autoResolution=broadAutoSelection?"broad_query_auto_recommendation":"commercial_default_recommendation";
  return {
    ok:true,
    product:winner.product,
    resolution:autoResolution,
    selection:{
      method:autoResolution,
      automatic:true,
      commercial_default:true,
      broad_query_auto_selection:broadAutoSelection,
      policy_version:"3.7.91",
...
async function rakutenAffiliateSearchAdmin(env,{query="",limit=100}={}){
  const q=String(query||"").trim();
  limit=Math.max(1,Math.min(100,Number(limit)||100));
  if(!q)return {service:"ANIME INTELLIGENCE",version:VERSION,ok:false,error:"query_required",query:q,returned:0,candidates:[]};
  const rows=await findProducts(env,q,limit);
  const candidates=(Array.isArray(rows)?rows:[]).slice(0,limit).map((p,i)=>({...rakutenCandidateView(p),search_position:i+1}));
  const product_type_counts={};
  for(const x of candidates){const t=String(x.product_type||"unknown");product_type_counts[t]=(product_type_counts[t]||0)+1;}
  return {service:"ANIME INTELLIGENCE",version:VERSION,ok:true,mode:"same_catalog_search_as_public_shop",query:q,returned:candidates.length,product_type_counts,candidates,note:"Searches the ANIME INTELLIGENCE canonical product database using the same findProducts() path as /shop. It does not scrape or query Rakuten search results. Use each candidate's Rakuten search link to generate an official affiliate URL, then register it back to the canonical product automatically."};
}
...
function compactPurchaseRouteAuditView(route){
  if(!route)return null;
  const pr=route.purchase_route||{};
  return {
    ...
    price_basis:route.price_basis||null,
    url:route.url||null,
    country_fit:pr.country_fit||null,
    country_fit_score:pr.country_fit_score??null,
    purchase_ease_score:pr.purchase_ease_score??null,
    overall_score:pr.overall_score??null,
    direct_shipping:pr.direct_shipping||null,
    directness:pr.directness||null,
    proxy_required:!!pr.proxy_required,
    language_support:pr.language_support||null,
    payment_compatibility:pr.payment_compatibility||null,
    account_difficulty:pr.account_difficulty||null,
    verification_blockers:pr.verification_blockers||[],
    recommended_action:pr.recommended_action||null,
    tradeoff:pr.tradeoff||null
  };
}

function repairLikelyUtf8Mojibake(value=""){
  const input=String(value||"");
  if(!input)return input;
  if(!/[\u0080-\u00ff]|(?:Ã|Ã|Ã£|Ã¥|Ã©|Â|Â|Â|Â|Â)/u.test(input))return input;
  try{
    const bytes=[];
    for(const ch of input){const cp=ch.codePointAt(0);if(cp>255)return input;bytes.push(cp);}
    const decoded=new TextDecoder("utf-8",{fatal:true}).decode(Uint8Array.from(bytes));
    return decoded&&decoded!==input?decoded:input;
  }catch{return input;}
}

async function countryAwareRoutingAudit(env,query="Hatsune Miku figure"){
  const raw=String(query||"").trim();
  const q=repairLikelyUtf8Mojibake(raw)||"Hatsune Miku figure";
  const u=new URL("https://audit.local/v1/best-place");
  u.searchParams.set("query",q);
  u.searchParams.set("buyer_country","JP");
  const preflight=await preflightPaidProduct(env,u);
  if(!preflight?.ok){
    return {service:"ANIME INTELLIGENCE",version:VERSION,audit:"COUNTRY_AWARE_PURCHASE_ROUTING_AUDIT",payment_required:false,query:q,all_pass:false,error:"product_resolution_failed",preflight:preflight?.body||null};
  }
  const product=preflight.product;
  let obs=[];try{obs=await getObservations(env,product.id);}catch{}
  let market_refresh_log=[];
  try{market_refresh_log=await refreshLiveMarketForProduct(env,product,obs);}catch(e){market_refresh_log=[{source:"market_refresh",ok:false,error:safeError(e)}];}
  try{obs=await getObservations(env,product.id);}catch{}
  let rakuten;try{rakuten=await rakutenSearch(env,product);}catch(e){rakuten={configured:rakutenConfigured(env),mode:rakutenWebServiceConfigured(env)?"live_web_service_error":"affiliate_link_only",web_service_api:rakutenWebServiceConfigured(env),offers:[],best:null,live_returned:0,live_matches:0,live_attempts:[],error:safeError(e)};}
  const fxInfo=await resolveUsdJpy(env),officialOffers=officialPurchaseOffersForProduct(product,fxInfo);
  ...
  for(const country of countries){
    ...
    results.push({
      ...
      practical_direct_candidate_count:routing.practical_direct_candidate_count||0,
      ...
      lowest_known_practical_route:compactPurchaseRouteAuditView(routing.lowest_known_practical_route),
      landed_cost:landed,
  ...
  try{
    const r=await fetch('/admin/finalize-v3',{
      ...
      headers:{
        ...
        'x-anime-intelligence-test':'admin_ui'
      }
    });
...
async function runX402ProductionE2E(ev){
  ...
  try{
    ...
    const r=await paidFetch(targetUrl,{method:'GET',headers:{'cache-control':'no-cache','x-anime-intelligence-test':'admin_x402_e2e'}});
...
export default{
  async fetch(request,env,ctx){
    ...
    try{
      ...
      if(url.pathname==="/shop/affiliate-click"&&request.method==="POST")return handlePublicAffiliateMetric(env,url,"affiliate_click");
      // Legacy redirect is kept for old already-issued API URLs, but new API responses no longer emit it.
      ...
      if(url.pathname.startsWith("/admin/")){
        ...
        if(url.pathname==="/admin/rakuten-affiliate/search"&&request.method==="GET")return json(await rakutenAffiliateSearchAdmin(env,{query:url.searchParams.get("query")||"",limit:url.searchParams.get("limit")||100}));
        ...
        if(url.pathname==="/admin/country-routing-audit"&&request.method==="GET")return json(await countryAwareRoutingAudit(env,url.searchParams.get("query")||"Hatsune Miku figure"));
