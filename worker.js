...
async function revenueMetrics(env){
  ...
  const externalSelected=selected.filter(isExternalCommercialEvent),externalGateEntered=gateEntered.filter(isExternalCommercialEvent),externalConfigErrors=configErrors.filter(isExternalCommercialEvent),externalPaymentRequired=paymentRequired.filter(isExternalCommercialEvent),externalAttempts=attempts.filter(isExternalCommercialEvent),externalVerified=verified.filter(isExternalCommercialEvent);
  ...
  return {
    ...
    measurement_note:"External funnel metrics exclude crawler/discovery probes, explicit admin/self-tests, configured test payers, and the bounded legacy self-test fingerprint window. payment_attempt requires a payment-signature retry. Raw diagnostic counts remain visible separately.",generated_at:new Date().toISOString()
  };
}
