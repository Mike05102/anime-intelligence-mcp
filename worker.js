...
function publicProductName(p){return cleanNullableTitle(p?.canonical_name_ja)||cleanNullableTitle(p?.canonical_name_en)||"åå";}
function publicProductCard(p,origin){const name=publicProductName(p),img=p?.official_image_url?`<img class="thumb" src="${publicHtmlEscape(p.official_image_url)}" alt="${publicHtmlEscape(name)}">`:"";return `<div class="card"><div class="grid"><div>${img}</div><div><b>${publicHtmlEscape(name)}</b><div class="meta">${publicHtmlEscape(p?.product_type||"")} ${p?.manufacturer?" / "+publicHtmlEscape(p.manufacturer):""}${p?.jan_code?" / JAN "+publicHtmlEscape(p.jan_code):""}</div><a class="btn btn2" href="${origin}/shop/product/${encodeURIComponent(p.id)}">ååæå ±ãè¦ã</a></div></div></div>`;}
async function publicShopLanding(env,origin,url){const q=String(url.searchParams.get("q")||"").trim();let body=`<h1>ã¢ãã¡ã°ããºãæ¢ã</h1><form method="get" action="/shop"><input name="q" value="${publicHtmlEscape(q)}" placeholder="ä¾ï¼åé³ãã¯ ãã£ã®ã¥ã¢ / ONE PIECE ã°ããº"><button type="submit">æ¤ç´¢</button></form>`;if(q){const rows=await findProducts(env,q,20);body+=`<h2>æ¤ç´¢çµæ ${rows.length}ä»¶</h2>`+(rows.length?rows.map(p=>publicProductCard(p,origin)).join(""):`<div class="card">è©²å½ååãè¦ã¤ããã¾ããã§ããã</div>`);}else{body+=`<div class="card"><b>å¬éã¡ãã£ã¢</b><p class="meta">AIã¨ã¼ã¸ã§ã³ãããæ¡åãããååãã¼ã¸ããæ¤ç´¢çµæããåååã®è³¼å¥æå ±ãç¢ºèªã§ãã¾ããæ¥½å¤©ã¢ãã£ãªã¨ã¤ãç»é²æ¸ã¿ã®ååã§ã¯æ¥½å¤©å¸å ´ã¸ã®ãªã³ã¯ãè¡¨ç¤ºãã¾ãã</p></div>`;}return publicShopShell("ã¢ãã¡ã°ããºæ¤ç´¢",body);}
async function publicShopProduct(env,origin,productId,ctx=null){const product=await loadProductById(env,productId);if(!product)return null;const name=publicProductName(product),offers=registeredRakutenAffiliateOffers(product).filter(x=>isOfficialRakutenAffiliateUrl(x.affiliate_url)),obs=await getObservations(env,product.id),market=marketView(product,obs),img=product.official_image_url?`<img class="thumb" src="${publicHtmlEscape(product.official_image_url)}" alt="${publicHtmlEscape(name)}">`:"";let body=`<p><a href="/shop">â ååæ¤ç´¢ã¸</a></p><div class="card"><div class="grid"><div>${img}</div><div><h1>${publicHtmlEscape(name)}</h1><div class="meta">${publicHtmlEscape(product.product_type||"")}${product.manufacturer?" / "+publicHtmlEscape(product.manufacturer):""}${product.jan_code?" / JAN "+publicHtmlEscape(product.jan_code):""}</div>${market.lowest_total_price_jpy!=null?`<p class="price">è¦³æ¸¬æå® Â¥${Number(market.lowest_total_price_jpy).toLocaleString("ja-JP")}</p><div class="meta">Yahoo/eBayç­ã®ç´è¿è¦³æ¸¬ãéæç­ãæªç¢ºèªã®å ´åãããã¾ãã</div>`:"<p class=\"meta\">ç¾å¨ä¾¡æ ¼ã®ç¢ºå®æå ±ã¯ããã¾ããã</p>"}</div></div></div>`;if(offers.length){body+=`<h2>æ¥½å¤©å¸å ´</h2>`+offers.slice(0,10).map((o,i)=>`<div class="card"><b>${publicHtmlEscape(o.title||o.seller||"æ¥½å¤©å¸å ´ã®åå")}</b>${o.price_jpy?`<div class="price">Â¥${Number(o.price_jpy).toLocaleString("ja-JP")}</div>`:""}<div class="meta">ç»é²æ¸ã¿æ¥½å¤©ã¢ãã£ãªã¨ã¤ããªã³ã¯${o.registered_at?" / ç»é²æ¥ "+publicHtmlEscape(String(o.registered_at).slice(0,10)):""}</div><a class="btn" href="${publicHtmlEscape(o.affiliate_url)}" onclick="fetch('/shop/affiliate-click?product_id=${encodeURIComponent(product.id)}',{method:'POST',keepalive:true}).catch(()=>{})">æ¥½å¤©å¸å ´ã§è¦ã</a></div>`).join("");if(ctx)deferTelemetry(ctx,logEvent(env,"affiliate_link_served",{endpoint:"/shop/product",product_id:product.id,metadata:{source:"rakuten",mode:"public_registered_media",count:offers.length,version:VERSION}}));}else{body+=`<div class="card"><b>æ¥½å¤©ãªã³ã¯ã¯æªç»é²ã§ã</b><p class="meta">æ¥½å¤©å¸å ´ã®ç»é²æ¸ã¿ã¢ãã£ãªã¨ã¤ããªã³ã¯ãããååã ãããã®å¬éãã¼ã¸ã«è¡¨ç¤ºãã¾ãã</p></div>`;}if(market.best_place?.url&&market.best_place?.source!=="rakuten")body+=`<h2>ãã®ä»ã®å¸å ´</h2><div class="card"><b>${publicHtmlEscape(market.best_place.source||"market")}</b><div class="price">Â¥${Number(market.best_place.total_price_jpy||market.best_place.price_jpy||0).toLocaleString("ja-JP")}</div><a class="btn btn2" href="${publicHtmlEscape(market.best_place.url)}">è²©å£²ãã¼ã¸ãè¦ã</a></div>`;return publicShopShell(name,body);}
...
export default{
  async fetch(request,env,ctx){
    ...
    try{
      ...
      if(url.pathname.startsWith("/shop/product/")&&request.method==="GET"){const productId=decodeURIComponent(url.pathname.slice("/shop/product/".length));const page=await publicShopProduct(env,origin,productId,ctx);return page?htmlResponse(page,200,{"cache-control":"public, max-age=60"}):htmlResponse(publicShopShell("ååãè¦ã¤ããã¾ãã","<div class=\"card\">ååãè¦ã¤ããã¾ããã§ããã</div>"),404,{"cache-control":"no-store"});}
