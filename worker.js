...
async function revenueMetrics(env){
  ...
  const realShoppingIntentCalls=queryCalls.filter(isExternalCommercialEvent);
  ...
  return {
    ...
    measurement_note:"Confirmed external shopping funnel requires a query/canonical-id signal (or the same request_id) and excludes crawler/discovery probes, explicit admin/self-tests, configured test payers, and the bounded legacy self-test fingerprint window. Bare non-test x402 probes are reported separately as non_test_probe_*. payment_attempt requires a payment-signature retry.",generated_at:new Date().toISOString()
  };
}
...
async function publicShopProduct(env,origin,productId,ctx=null){const product=await loadProductById(env,productId);if(!product)return null;const name=publicProductName(product),offers=registeredRakutenAffiliateOffers(product).filter(x=>isOfficialRakutenAffiliateUrl(x.affiliate_url)),obs=await getObservations(env,product.id),market=marketView(product,obs),img=product.official_image_url?`<img class="thumb" src="${publicHtmlEscape(product.official_image_url)}" alt="${publicHtmlEscape(name)}">`:"";let body=`<p><a href="/shop">\u2190 \u5546\u54c1\u691c\u7d22\u3078</a></p><div class="card"><div class="grid"><div>${img}</div><div><h1>${publicHtmlEscape(name)}</h1><div class="meta">${publicHtmlEscape(product.product_type||"")}${product.manufacturer?" / "+publicHtmlEscape(product.manufacturer):""}${product.jan_code?" / JAN "+publicHtmlEscape(product.jan_code):""}</div>${market.lowest_total_price_jpy!=null?`<p class="price">\u89b3\u6e2c\u6700\u5b89 \u00a5${Number(market.lowest_total_price_jpy).toLocaleString("ja-JP")}</p><div class="meta">Yahoo/eBay\u7b49\u306e\u76f4\u8fd1\u89b3\u6e2c\u3002\u9001\u6599\u7b49\u304c\u672a\u78ba\u8a8d\u306e\u5834\u5408\u304c\u3042\u308a\u307e\u3059\u3002</div>`:"<p class=\"meta\">\u73fe\u5728\u4fa1\u683c\u306e\u78ba\u5b9a\u60c5\u5831\u306f\u3042\u308a\u307e\u305b\u3093\u3002</p>"}</div></div></div>`;if(offers.length){body+=`<h2>\u697d\u5929\u5e02\u5834</h2>`+offers.slice(0,10).map((o,i)=>`<div class="card"><b>${publicHtmlEscape(o.title||o.seller||"\u697d\u5929\u5e02\u5834\u306e\u5546\u54c1")}</b>${o.price_jpy?`<div class="price">\u00a5${Number(o.price_jpy).toLocaleString("ja-JP")}</div>`:""}<div class="meta">\u767b\u9332\u6e08\u307f\u697d\u5929\u30a2\u30d5\u30a3\u30ea\u30a8\u30a4\u30c8\u30ea\u30f3\u30af${o.registered_at?" / \u767b\u9332\u65e5 "+publicHtmlEscape(String(o.registered_at).slice(0,10)):""}</div><a class="btn" href="${publicHtmlEscape(o.affiliate_url)}" onclick="fetch('/shop/affiliate-click?product_id=${encodeURIComponent(product.id)}',{method:'POST',keepalive:true}).catch(()=>{})">\u697d\u5929\u5e02\u5834\u3067\u898b\u308b</a></div>`).join("");if(ctx)deferTelemetry(ctx,logEvent(env,"affiliate_link_served",{endpoint:"/shop/product",product_id:product.id,metadata:{source:"rakuten",mode:"public_registered_media",count:offers.length,version:VERSION}}));}else{body+=`<div class="card"><b>\u697d\u5929\u30ea\u30f3\u30af\u306f\u672a\u767b\u9332\u3067\u3059</b><p class="meta">\u697d\u5929\u5e02\u5834\u306e\u767b\u9332\u6e08\u307f\u30a2\u30d5\u30a3\u30ea\u30a8\u30a4\u30c8\u30ea\u30f3\u30af\u304c\u3042\u308b\u5546\u54c1\u3060\u3051\u3001\u3053\u306e\u516c\u958b\u30da\u30fc\u30b8\u306b\u8868\u793a\u3057\u307e\u3059\u3002</p></div>`;}if(market.best_place?.url&&market.best_place?.source!=="rakuten")body+=`<h2>\u305d\u306e\u4ed6\u306e\u5e02\u5834</h2><div class="card"><b>${publicHtmlEscape(market.best_place.source||"market")}</b><div class="price">\u00a5${Number(market.best_place.total_price_jpy||market.best_place.price_jpy||0).toLocaleString("ja-JP")}</div><a class="btn btn2" href="${publicHtmlEscape(market.best_place.url)}">\u8ca9\u58f2\u30da\u30fc\u30b8\u3092\u898b\u308b</a></div>`;return publicShopShell(name,body);}
async function handlePublicAffiliateClick(env,url){const pid=String(url.searchParams.get("product_id")||"").trim()||null;await logEvent(env,"affiliate_click",{endpoint:"/shop/affiliate-click",product_id:pid,metadata:{source:"rakuten",mode:"public_registered_media",version:VERSION}});return new Response(null,{status:204,headers:corsHeaders({"cache-control":"no-store"})});}
...
export default{
  async fetch(request,env,ctx){
    ...
    try{
      ...
      if(url.pathname==="/shop/affiliate-click"&&request.method==="POST")return handlePublicAffiliateClick(env,url);
