...
async function multilingualAmbiguousShoppingE2EAudit(env,url){
  ...
  try{
    ...
    // 3) Merchandise subtype such as NARUTO T-shirt: query franchise + subtype aliases globally, then validate.
    if(c.subtype&&!rows.length){
      const terms=[preferredFranchiseSearchAlias(intent?.franchises?.[0]||""),preferredTypeSearchAlias(intent),...(intent?.merch_subtypes||[])];
      const got=await auditGlobalTitleFetch(env,terms,70);
      rows=got.filter(p=>candidateMatchesIntentFranchises(p,intent.franchises)&&candidateMatchesMerchSubtype(p,intent.merch_subtypes));
    }

    // 4) If a requested preference has no evidence in the pool, perform one small evidence rescue.
    const prefKey=c.pref||null;
    if(prefKey){
      const best=Math.max(0,...rows.map(p=>candidatePreferenceFit(p,intent?.preferences||[]).matched.length));
      if(best===0){
        const terms=[...(intent?.characters?.length?auditCharacterSearchTerms(intent.characters[0]).slice(0,2):[preferredFranchiseSearchAlias(intent?.franchises?.[0]||"")]),...auditPreferenceEvidenceTerms(intent).slice(0,5)];
        const got=await auditGlobalTitleFetch(env,terms,80);
        const compatible=got.filter(p=>intentCompatibility(p,intent).ok);
