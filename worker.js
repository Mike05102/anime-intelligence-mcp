...
function candidatePreferenceFit(p,preferences=[]){
  ...
  for(const pref of preferences||[]){let ok=false;
    ...
    else if(pref.facet==="use_case"&&pref.value==="gift")ok=!!p?.official_image_url&&!!(p?.jan_code||p?.official_url);
...
function candidateIntentBonus(p,intent){
  ...
  const prefFit=candidatePreferenceFit(p,intent?.preferences||[]);b.ambiguous_preference_fit=prefFit;score+=prefFit.score;
