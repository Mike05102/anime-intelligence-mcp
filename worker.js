...
async function auditPreferenceSpecificFetch(env,intent,limit=80){
  // Preference evidence must be the search anchor, not OR'ed with the character name.
  // Otherwise a plain Luffy/Zoro/etc. result can satisfy the retrieval while completely
  // ignoring "sealed", "red", "black", "Japan-only", etc.
  const evidence=auditPreferenceEvidenceTerms(intent).slice(0,4);
  const groups=[];
  for(const term of evidence.slice(0,2)){
    const rows=await auditStructuredCatalogFetch(env,intent,{limit:Math.min(60,limit),titleTerm:term,skipType:!!intent?.merch_subtypes?.length});
...
async function multilingualAmbiguousShoppingE2EAudit(env,url){
  ...
  try{
    ...
    if(c.subtype&&!rows.length){
      ...
      const got=await auditStructuredCatalogFetch(env,intent,{limit:70,titleTerm:subtypeTerm,skipType:true});
