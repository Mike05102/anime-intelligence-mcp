...
async function multilingualAmbiguousShoppingE2EAudit(env,url){
  ...
  if(expectedContextStop){
    ...
    return {service:"ANIME INTELLIGENCE",version:VERSION,audit:"MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_AUDIT_CASE",resource_safe_mode:"one lightweight structured catalog case per Worker request; no wildcard scan/self-discovery/x402/payment",payment_required:false,real_payment_test_required:false,index,total:MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_CASES.length,result};
  }
  ...
  try{
    ...
    if(character){
      ...
      for(const term of aliases.slice(0,2)){
        const got=await auditStructuredCatalogFetch(env,intent,{limit:45,mustTitleTerm:term});
        const compatible=got.filter(p=>intentCompatibility(p,intent).ok);
        if(compatible.length){rows=compatible;break;}
      }
    }else if(c.subtype){
      const term=preferredTypeSearchAlias(intent);
      rows=(await auditStructuredCatalogFetch(env,intent,{limit:45,skipType:true,mustTitleTerm:term})).filter(p=>intentCompatibility(p,intent).ok);
    }else{
      rows=(await auditStructuredCatalogFetch(env,intent,{limit:55})).filter(p=>intentCompatibility(p,intent).ok);
    }
    ...
    if(prefKey&&rows.length){
      ...
      if(best===0){
        const evidence=auditPreferenceEvidenceTerms(intent).slice(0,3);
        if(evidence.length){
          const char=intent?.characters?.[0]||"";
          const charTerm=char?(auditCharacterSearchTerms(char)[0]||""):"";
          const extra=await auditStructuredCatalogFetch(env,intent,{limit:45,skipType:!!c.subtype,mustTitleTerm:charTerm,anyTitleTerms:evidence});
          rows=mergeUniqueProducts([rows,extra],100).filter(p=>intentCompatibility(p,intent).ok);
        }
      }
    }
    if(!rows.length&&c.subtype){
      const term=preferredTypeSearchAlias(intent);
      rows=(await auditStructuredCatalogFetch(env,intent,{limit:50,skipType:true,mustTitleTerm:term})).filter(p=>candidateMatchesIntentFranchises(p,intent.franchises)&&candidateMatchesMerchSubtype(p,intent.merch_subtypes));
    }
  ...
  return {service:"ANIME INTELLIGENCE",version:VERSION,audit:"MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_AUDIT_CASE",resource_safe_mode:"one lightweight structured catalog case per Worker request; max two bounded catalog reads; no wildcard scan/self-discovery/x402/payment",payment_required:false,real_payment_test_required:false,index,total:MULTILINGUAL_AMBIGUOUS_SHOPPING_E2E_CASES.length,result};
}
